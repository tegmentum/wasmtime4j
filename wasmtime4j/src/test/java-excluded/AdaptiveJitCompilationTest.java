 ...
 *
 * <p>Tests tier transitions, optimization strategy selection, hot spot detection, and performance
 * regression handling in the adaptive compilation engine.
 */
class AdaptiveJitCompilationTest {
  ...
  void setUp() {
    ...
    final AdaptiveOptimizationConfig config =
  ...
  void testTierTransitionProgression() {
    ...
    final FunctionExecutionTracker tracker =
        new FunctionExecutionTracker(functionId, JitCompilationStrategy.BASELINE);
    ...
    if (decision1.shouldTransition()) {
      assertTrue(
          decision1.getTargetStrategy().getTier() > currentStrategy.getTier(),
    ...
    if (decision2.shouldTransition()) {
      assertTrue(
          decision2.getTargetStrategy().getTier() >= JitCompilationStrategy.OPTIMIZED.getTier(),
  ...
  void testOptimizationStrategySelection() {
    ...
    final ExecutionProfile computeProfile =
    ...
    final List<OptimizationStrategy> strategies =
        strategyManager.selectStrategies(computeProfile, JitCompilationStrategy.HIGHLY_OPTIMIZED);

    assertFalse(strategies.isEmpty(), "Should select optimization strategies");

    // Verify that appropriate strategies are selected
    final boolean hasVectorization =
        strategies.stream().anyMatch(strategy -> strategy instanceof VectorizationOptimization);
    final boolean hasLoopOptimization =
        strategies.stream().anyMatch(strategy -> strategy instanceof LoopOptimization);

    assertTrue(
        hasVectorization || hasLoopOptimization,
    ...
    final List<OptimizationStrategy> baselineStrategies =
        strategyManager.selectStrategies(computeProfile, JitCompilationStrategy.BASELINE);

    assertTrue(
        baselineStrategies.size() <= strategies.size(),
        "Baseline should have fewer or equal strategies than highly optimized");

    LOGGER.info(
        "Selected {} strategies for highly optimized tier, {} for baseline",
        strategies.size(),
        baselineStrategies.size());
  }
  ...
  void testHotSpotDetection() throws InterruptedException {
    ...
    for (int i = 0; i < 2000; i++) {
      optimizationEngine.recordExecution(
          hotFunctionId, JitCompilationStrategy.BASELINE, 500_000 + (i % 100_000), 1024);
    }

    // Simulate cold function execution pattern
    for (int i = 0; i < 10; i++) {
      optimizationEngine.recordExecution(
          coldFunctionId, JitCompilationStrategy.BASELINE, 10_000_000, 512);
    }
    ...
    LOGGER.info(
        "Hot spot detection completed: {} optimizations, {} functions tracked",
        stats.getTotalOptimizations(),
        stats.getTrackedFunctionCount());
  }
  ...
  void testPerformanceRegressionDetection() {
    ...
    final FunctionExecutionTracker tracker =
        new FunctionExecutionTracker(functionId, JitCompilationStrategy.OPTIMIZED);
    ...
    assertFalse(
        goodStats.hasRecentPerformanceDegraded(),
    ...
    assertTrue(
        regressedStats.hasRecentPerformanceDegraded(), "Should detect performance regression");
  ...
  void testSystemLoadAdaptation() {
    ...
    assertTrue(
        adaptedConfig.getMediumFrequencyThreshold() >= baseConfig.getMediumFrequencyThreshold(),
        "Should increase thresholds under high load");

    LOGGER.info(
        "System load adaptation test completed with {} overhead and {} load",
        String.format("%.1f%%", overhead * 100),
        loadCategory);
  }
  ...
  void testOptimizationEffectiveness() {
    ...
    strategyManager.recordPerformanceFeedback(
        strategyName, expectedBenefit,
  ...
  void testConcurrentOptimization() throws InterruptedException {
      ...
      new Thread(
              () -> {
                try {
                  ...
                  for (int i = 0; i < 1000; i++) {
                    optimizationEngine.recordExecution(
                        functionId, JitCompilationStrategy.BASELINE, 1_000_000 + (i * 1000), 1024);
    ...
    final AdaptiveOptimizationStatistics stats = optimizationEngine.getStatistics();
    assertTrue(stats.getTrackedFunctionCount() >= functionIds.length, "Should track all functions");

    LOGGER.info(
        "Concurrent optimization test completed: {} functions tracked, {} optimizations",
        stats.getTrackedFunctionCount(),
        stats.getTotalOptimizations());
  }
  ...
  void testCompilationStrategyComparison() {
    ...
    final ExecutionProfile profile =
        new ExecutionProfile(
            1000, 10.0, 10000, 0.5, 1024 * 1024, true, false, false, 5, 2 * 1024 * 1024);
    ...
    for (final JitCompilationStrategy strategy : JitCompilationStrategy.values()) {
      ...
      final List<OptimizationStrategy> strategies =
          strategyManager.selectStrategies(profile, strategy);

      assertTrue(
          strategies.size() >= 0, "Should select valid number of strategies for " + strategy);

      // Higher tiers should generally have more optimization strategies
      if (strategy.getTier() > 0) {
        assertFalse(strategies.isEmpty(), "Higher tiers should have optimization strategies");
      }
    }
  ...
  void testEngineStateTransitions() throws InterruptedException {
    ...
    assertEquals(
        AdaptiveOptimizationEngine.EngineState.LEARNING,
        stats.getEngineState(),
        "Should start in LEARNING state");

    // Simulate enough execution to trigger transition to OPTIMIZING
    for (int i = 0; i < 10; i++) {
      optimizationEngine.recordExecution(
          "func_" + i, JitCompilationStrategy.BASELINE, 1_000_000, 1024);
    }
    ...
    assertTrue(
        stats.getEngineState() == AdaptiveOptimizationEngine.EngineState.LEARNING
