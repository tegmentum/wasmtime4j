...
class ExecutionTracerTest {
  ...
  @Mock private WasmDebugger mockDebugger;
  ...
  class PerformanceAnalysisTests {
    ...
    void performanceAnalysisWithMockData() {
      final PerformanceAnalysis analysis =
          new PerformanceAnalysis(Duration.ofSeconds(1), 10000, List.of(), 5);
      ...
      assertEquals("No significant performance bottlenecks identified", recommendations.get(0));
    }
  }
  ...
  class TraceExportTests {
    ...
    void chromeTraceFormatExport() {
      final TraceExport.ExportEvent event =
          new TraceExport.ExportEvent(
              java.time.Instant.now(),
              TraceEventType.FUNCTION_CALL,
              "Function call: main",
              Map.of("functionName", "main", "duration", "100ms"));

      final TraceExport export =
          new TraceExport(
              List.of(event), new TraceStatistics(1, 0, Duration.ZERO, Map.of(), 0), Map.of());
