 ...
 *
 * <p>This interface provides mapping from WebAssembly bytecode to original source code locations
 * when debug information or source maps are available.
 *
 * @since 1.0.0
 */
public interface SourceLocation {
  ...
  static SourceLocation of(final String fileName, final int lineNumber) {
    return builder().fileName(fileName).lineNumber(lineNumber).build();
  }
  ...
  static SourceLocation of(final String fileName, final int lineNumber, final int columnNumber) {
    return builder().fileName(fileName).lineNumber(lineNumber).columnNumber(columnNumber).build();
  }
}
