 ...
 *
 * <p>The Memory64 proposal extends table operations to support 64-bit indices and
 * table sizes, enabling tables to hold more than 4 billion elements. This is critical
 * for large-scale WebAssembly applications that need extensive function tables or
 * reference tables.
 *
...
public enum Memory64TableOperations {
    ...
    public void validateRange(final long startIndex, final long length, final long tableSize) {
        ...
        if (startIndex + length > tableSize) {
            throw new IndexOutOfBoundsException(
                String.format("Range [%d, %d) exceeds table size %d",
                    startIndex, startIndex + length, tableSize));
        }
    }
     ...
     *
     * <p>This provides a generic execution interface for table operations with
    ...
    public WasmValue execute(final WasmTable table, final Object... args) {
        ...
        switch (this) {
            ...
            case TABLE_FILL_64:
                if (args.length < 3) {
                    throw new IllegalArgumentException("TABLE_FILL_64 requires start, length, and value arguments");
                }
            ...
            case TABLE_COPY_64:
                if (args.length < 4) {
                    throw new IllegalArgumentException("TABLE_COPY_64 requires dest, src, and length arguments");
                }
            ...
            case TABLE_INIT_64:
                if (args.length < 4) {
                    throw new IllegalArgumentException("TABLE_INIT_64 requires dest, src, length, and segment arguments");
                }
