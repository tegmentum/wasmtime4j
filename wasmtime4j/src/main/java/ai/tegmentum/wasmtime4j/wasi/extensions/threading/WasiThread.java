 ...
 *
 * <p>A WasiThread provides control over thread execution, lifecycle management, and state
 * monitoring. Threads are created through the {@link
 * ai.tegmentum.wasmtime4j.wasi.extensions.WasiThreading} interface and can be started, joined, and
 * interrupted.
 *
 * <p>Thread lifecycle:
 *
 * <ol>
 *   <li>NEW - Thread created but not started
 *   <li>RUNNABLE - Thread is executing or ready to execute
 *   <li>BLOCKED - Thread is blocked waiting for a monitor lock
 *   <li>WAITING - Thread is waiting indefinitely for another thread
 *   <li>TIMED_WAITING - Thread is waiting for a specified period
 *   <li>TERMINATED - Thread has completed execution
 * </ol>
 *
 * <p>Example usage:
 *
...
public interface WasiThread {
   ...
   *
   * <p>The thread begins executing the target Runnable that was specified when the thread was
   * created. A thread can only be started once.
   *
   ...
   *
   * <p>This method blocks the calling thread until this thread completes execution. If this thread
   * has already terminated, this method returns immediately.
   *
   ...
   *
   * <p>This method blocks the calling thread for up to the specified timeout period waiting for
   * this thread to complete. If the timeout elapses before the thread terminates, the method
   * returns false.
   *
   ...
   *
   * <p>This method sets the interrupted status of the thread and may cause the thread to throw an
   * InterruptedException if it's blocked in certain operations (sleep, wait, join, etc.).
   *
   ...
   *
   * <p>This method returns the interrupted status of the thread but does not clear the interrupted
   * status.
   *
   ...
   *
   * <p>The thread name is used for debugging and monitoring purposes. It should be descriptive of
   ...
   *
   * <p>Thread priority is a hint to the thread scheduler about the relative importance of threads.
   * Higher priority threads may receive more CPU time.
   *
   ...
   *
   * <p>Priority changes may not take effect immediately and the actual scheduling behavior depends
   ...
   *
   * <p>A thread is alive if it has been started and has not yet terminated. This includes threads
   * that are currently running, blocked, or waiting.
   *
   ...
   *
   * <p>Daemon threads do not prevent the JVM from exiting when all non-daemon threads have
   * terminated.
   *
   ...
   *
   * <p>This method must be called before the thread is started. Daemon threads do not prevent
   ...
   *
   * <p>Returns the approximate stack size in bytes. A value of 0 indicates that the stack size is
   * system-dependent or unknown.
   *
   ...
   *
   * <p>Returns detailed information about thread execution including CPU time, blocked time, and
   ...
   *
   * <p>The handler will be called when this thread terminates due to an uncaught exception.
   *
   * @param handler the uncaught exception handler
   * @throws WasmException if handler setting fails
   */
  void setUncaughtExceptionHandler(final Thread.UncaughtExceptionHandler handler)
