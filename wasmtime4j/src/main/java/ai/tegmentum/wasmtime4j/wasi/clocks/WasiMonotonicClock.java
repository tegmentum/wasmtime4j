 ...
 * non-decreasing values. The clock measures time in nanoseconds relative to an unspecified initial
 * value.
 *
 * <p>This interface should be used for measuring elapsed time, performance timing, and implementing
 * timeouts. For reporting the current date and time to humans, use {@link WasiWallClock} instead.
 *
...
public interface WasiMonotonicClock {
   ...
   *
   * <p>The returned value is in nanoseconds relative to an unspecified initial value. Successive
   * calls to this method will return non-decreasing values.
   *
   ...
   *
   * <p>The resolution represents the duration of a single clock tick in nanoseconds. This indicates
   ...
   *
   * <p>The pollable will become ready when the monotonic clock reaches or exceeds the specified
   * instant. This can be used to implement timeouts and scheduled tasks.
   *
   ...
   *
   * <p>The pollable will become ready after the given duration has passed from the time this method
   * is called. This is useful for implementing delays and timeouts.
   *
