 ...
 *
 * <ul>
 *   <li>Thread creation and management
 *   <li>Synchronization primitives (mutexes, condition variables, semaphores)
 *   <li>Thread pools and async execution
 *   <li>Inter-thread communication
 * </ul>
 *
 * <p>All threading operations respect capability-based security and require appropriate permissions
 * to be granted through the WASI security policy. Thread creation and synchronization operations
...
public interface WasiThreading {
   ...
   *
   * <p>The thread is created in a suspended state and must be explicitly started using {@link
   * WasiThread#start()}. The target Runnable will be executed in the context of the new thread.
   *
   ...
   *
   * <p>Named threads are useful for debugging and monitoring. The name should be descriptive of the
   * thread's purpose.
   *
   * @param target the code to execute in the new thread
   ...
   *
   * <p>Mutexes provide exclusive access to shared resources, ensuring that only one thread can hold
   * the lock at a time. They support both blocking and non-blocking lock acquisition.
   *
   ...
   *
   * <p>Condition variables allow threads to wait for specific conditions and be notified when those
   * conditions are met. They must be used in conjunction with a mutex.
   *
   ...
   *
   * <p>Semaphores control access to a limited number of resources by maintaining a count of
   * available permits. Threads can acquire and release permits to access the protected resources.
   *
   ...
   *
   * <p>Thread pools manage a collection of worker threads that can execute tasks submitted to them.
   * They provide efficient task execution by reusing threads rather than creating new ones for each
   * task.
   *
   ...
   *
   * <p>This method provides a convenient way to execute tasks asynchronously without explicitly
   * managing thread pools. The task is executed in a background thread and the returned
   * CompletableFuture can be used to track completion.
   *
   ...
   *
   * <p>Similar to {@link #executeAsync(Runnable)} but for tasks that return a value. The result is
   * available through the returned CompletableFuture.
   *
   ...
   *
   * <p>This method returns a WasiThread representation of the currently executing thread, allowing
   * access to WASI-specific thread operations.
   *
   ...
   *
   * <p>This information can be used to determine optimal thread pool sizes and parallelism levels
   ...
   *
   * <p>This limit is imposed by the WASI security policy and system resources. Applications should
   * respect this limit to avoid resource exhaustion.
   *
   ...
   *
   * <p>This includes all threads created through this threading interface that are currently alive
   ...
   *
   * <p>This method can be used to verify that the threading extension is functional and that the
   ...
   *
   * <p>This method returns information about thread usage, synchronization statistics, and
   * performance metrics for the threading subsystem.
   *
   ...
   *
   * <p>This is a convenience method for thread sleep operations that respects WASI timing
   * constraints and interruption policies.
   *
   ...
   *
   * <p>This is a hint to the thread scheduler that the current thread is willing to give up its
