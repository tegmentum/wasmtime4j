 ...
 *
 * <p>This enum defines the types of evolution operations that can be performed
 * on WIT interfaces while maintaining compatibility and semantic correctness.
 *
 * @since 1.0.0
 */
public enum WitEvolutionOperation {
   ...
   */
  WitEvolutionOperation(final String description, final boolean breaking, final OperationComplexity complexity) {
  ...
  public OperationCategory getCategory() {
    ...
    } else if (name.contains("TYPE") || name.contains("FIELD") || name.contains("VARIANT") || name.contains("ENUM")) {
  ...
  public MigrationEffort getEstimatedEffort() {
    return switch (complexity) {
      case LOW -> breaking ? MigrationEffort.MEDIUM : MigrationEffort.LOW;
      case MEDIUM -> breaking ? MigrationEffort.HIGH : MigrationEffort.MEDIUM;
      case HIGH -> breaking ? MigrationEffort.VERY_HIGH : MigrationEffort.HIGH;
    };
  }
