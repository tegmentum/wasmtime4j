 ...
 *
 * <p>This interface defines retry strategies for handling transient failures in WebAssembly
 * execution, with configurable delays, conditions, and limits.
 *
 * @since 1.0.0
 */
public interface RetryPolicy {
  ...
  static RetryPolicy fixedDelay(int maxAttempts, Duration delay) {
    return builder().maxAttempts(maxAttempts).strategy(Strategy.FIXED).baseDelay(delay).build();
  }
