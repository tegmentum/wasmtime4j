...
public interface FutureAny extends AutoCloseable {
  ...
  static FutureAny create(final long handle) {
    ...
    return new DefaultFutureAny(handle, null);
  }

  /**
   * Creates a new future handle wrapping the given handle ID with a native close action.
   *
   ...
   * @throws IllegalArgumentException if handle is not positive
   */
  static FutureAny create(final long handle, final Runnable closeAction) {
    if (handle <= 0) {
      throw new IllegalArgumentException("handle must be positive, got: " + handle);
    }
  ...
  final class DefaultFutureAny implements FutureAny {
    ...
    DefaultFutureAny(final long handle, final Runnable closeAction) {
      this.handle = handle;
    ...
    public String toString() {
      return "FutureAny{handle=" + handle + ", valid=" + !closed.get() + "}";
    }
