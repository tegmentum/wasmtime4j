 ...
 * 
 * <p>The runtime abstracts away the underlying implementation details (JNI vs Panama FFI)
 * and provides a consistent API across different Java versions and native bindings.
 * 
 * <p>Example usage:
 * <pre>{@code
...
public interface WasmRuntime extends Closeable {
     ...
     * 
     * <p>The engine is responsible for compilation and execution of WebAssembly modules.
     * Multiple engines can be created from a single runtime, allowing for isolated
     * execution contexts.
     * 
     ...
     * 
     * <p>This method performs validation and compilation of the WebAssembly bytecode.
     * The resulting module can be instantiated multiple times across different engines.
     * 
     ...
     * 
     * <p>This method instantiates a compiled module, making its exported functions
     * and memory available for use. Each instance represents a separate execution
     * context with its own memory and global state.
     * 
     ...
     * 
     * <p>After calling this method, the runtime becomes invalid and should not be used.
     * Any attempt to use the runtime after closing may result in exceptions.
     */
    @Override
    void close();
}