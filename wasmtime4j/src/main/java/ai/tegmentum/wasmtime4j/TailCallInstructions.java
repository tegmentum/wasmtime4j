 ...
 *
 * <p>The tail call proposal adds proper tail call elimination to WebAssembly,
 * enabling:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Tail calls replace the current function's stack frame instead of creating
 * a new one, enabling unlimited recursion depth for tail-recursive functions.
 *
 * <p>Example usage:
 * <pre>{@code
...
public enum TailCallInstructions {
    ...
    /**
     * Tail call to a function by index.
     * Replaces the current stack frame with a call to the target function.
     */
    TAIL_CALL(0x12, "tail_call", true, false),

    /**
     * Tail call indirect through a function table.
     * Replaces the current stack frame with an indirect call.
     */
    TAIL_CALL_INDIRECT(0x13, "tail_call_indirect", true, true),

    /**
     * Return call - performs a tail call and immediately returns.
     * Equivalent to tail_call followed by return.
     */
    RETURN_CALL(0x14, "return_call", false, false),

    /**
     * Return call indirect - performs an indirect tail call and returns.
     * Equivalent to tail_call_indirect followed by return.
     */
    ...
    TailCallInstructions(final int opcode, final String mnemonic,
                        final boolean replacesFrame, final boolean isIndirect) {
        this.opcode = opcode;
     ...
     *
     * <p>This method provides a generic execution interface for tail call
     * instructions with proper stack frame management.
     *
     ...
     */
    public WasmValue[] execute(final TailCallContext context,
                              final Object target,
                              final WasmValue... parameters) throws WasmException {
        ...
        switch (this) {
            ...
            case RETURN_CALL_INDIRECT:
                final WasmValue[] indirectResults = executeTailCallIndirect(
                    context, (TailCallIndirectTarget) target, parameters);
    ...
    private WasmValue[] executeTailCall(final TailCallContext context,
    ...
    private WasmValue[] executeTailCallIndirect(final TailCallContext context,
                                               final TailCallIndirectTarget target,
                                               final WasmValue... parameters) throws WasmException {

        final FunctionReference function = context.resolveIndirectFunction(
    ...
    private void validateTailCallParameters(final FunctionType targetType,
                                          final WasmValue... parameters) throws WasmException {

        final WasmValueType[] expectedTypes = targetType.getParameterTypes();
     ...
     */
    public static boolean canOptimizeAsTailCall(final FunctionContext currentFunction,
     ...
     */
    public static int estimateStackSavings(final FunctionContext currentFunction,
                                          final FunctionContext targetFunction) {
    ...
    public static class TailCallIndirectTarget {
        ...
        public TailCallIndirectTarget(final int tableIndex,
                                    final int functionIndex,
