 ...
 * 
 * <p>This factory automatically selects the appropriate runtime implementation
 * based on the Java version and available native bindings. It can also be
 * configured to use a specific runtime type.
 * 
 * <p>Runtime selection priority:
 * <ol>
 *   <li>Manual override via system property: {@code wasmtime4j.runtime}</li>
 *   <li>Panama FFI for Java 23+ (if available)</li>
 *   <li>JNI fallback for all Java versions</li>
 * </ol>
 * 
 * @since 1.0.0
 */
public final class WasmRuntimeFactory {
     ...
     * 
     * <p>This method automatically selects the best available runtime implementation
     * based on the Java version and system configuration. It prefers Panama FFI
     * on Java 23+ but falls back to JNI if Panama is not available.
     * 
    ...
    public static WasmRuntime create(final RuntimeType runtimeType) throws WasmException {
        ...
        logger.info("Creating WebAssembly runtime with type: " + runtimeType);
    ...
    public static boolean isRuntimeAvailable(final RuntimeType runtimeType) {
        try {
            ...
            logger.warning("Error checking runtime availability for " + runtimeType + ": " + e.getMessage());
            return false;
        }
    }
    ...
    private static RuntimeType selectRuntimeType() {
        ...
        if (override != null) {
            if (RUNTIME_JNI.equalsIgnoreCase(override)) {
                ...
                logger.warning("Unknown runtime type in system property: " + override + ", using automatic selection");
            }
        }
        ...
        if (javaVersion >= 23 && isPanamaRuntimeAvailable()) {
            logger.info("Auto-selected Panama runtime for Java " + javaVersion);
            return RuntimeType.PANAMA;
        } else {
            logger.info("Auto-selected JNI runtime for Java " + javaVersion);
            return RuntimeType.JNI;
        }
    }
    ...
    private static WasmRuntime createPanamaRuntime() throws WasmException {
        try {
            ...
            final Class<?> panamaRuntimeClass = Class.forName("ai.tegmentum.wasmtime4j.panama.PanamaWasmRuntime");
