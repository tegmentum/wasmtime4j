 ...
 *
 * <p>This class implements the WebAssembly function subtyping rules as specified
 * in the typed function references proposal:
 *
 * <ul>
...
public final class FunctionTypeValidator {
    ...
    public static boolean isSubtype(final FunctionType subtype, final FunctionType supertype) {
        ...
        final WasmValueType[] subParams = subtype.getParameterTypes();
        final WasmValueType[] superParams = supertype.getParameterTypes();
        if (subParams.length != superParams.length) {
            return false;
        }

        // Check result count
        final WasmValueType[] subResults = subtype.getResultTypes();
        final WasmValueType[] superResults = supertype.getResultTypes();
     ...
     *
     * <p>Compatibility is more lenient than subtyping and allows for safe
     * function calls even if types aren't in a perfect subtype relationship.
     *
     ...
     */
    private static boolean isWeaklyCompatible(final FunctionType expected, final FunctionType actual) {
        // Check parameter and result counts
        if (expected.getParameterTypes().length != actual.getParameterTypes().length
            || expected.getResultTypes().length != actual.getResultTypes().length) {
            return false;
        }

        // Check if types can be safely coerced
        final WasmValueType[] expectedParams = expected.getParameterTypes();
        final WasmValueType[] actualParams = actual.getParameterTypes();
        ...
        final WasmValueType[] expectedResults = expected.getResultTypes();
        final WasmValueType[] actualResults = actual.getResultTypes();
     ...
     */
    private static boolean checkReferenceSubtyping(final WasmValueType subtype, final WasmValueType supertype) {
        ...
        // For typed function references, check function type subtyping
        if (subtype instanceof TypedFunctionReferenceType && supertype instanceof TypedFunctionReferenceType) {
            final TypedFunctionReferenceType subtypedFunc = (TypedFunctionReferenceType) subtype;
            final TypedFunctionReferenceType supertypedFunc = (TypedFunctionReferenceType) supertype;
            return isSubtype(subtypedFunc.getFunctionType(), supertypedFunc.getFunctionType());
        }
    ...
    private static boolean isReferenceType(final WasmValueType type) {
        return type == WasmValueType.FUNCREF
            ...
            || type instanceof TypedFunctionReferenceType
            || type instanceof ExtendedReferenceType;
    }
    ...
    public static void validateCall(
            final FunctionType expectedType,
            final FunctionType actualType,
            final WasmValue[] parameters) throws ai.tegmentum.wasmtime4j.exception.ValidationException {

        if (!isCompatible(expectedType, actualType)) {
            throw new ai.tegmentum.wasmtime4j.exception.ValidationException(
                String.format("Function type mismatch: expected %s, got %s",
                    expectedType, actualType));
        }

        // Validate parameter count
        if (parameters.length != expectedType.getParameterTypes().length) {
            throw new ai.tegmentum.wasmtime4j.exception.ValidationException(
                String.format("Parameter count mismatch: expected %d, got %d",
                    expectedType.getParameterTypes().length, parameters.length));
        }

        // Validate parameter types
        final WasmValueType[] expectedParams = expectedType.getParameterTypes();
    ...
    public static void validateAssignment(
            final FunctionType targetType,
            final FunctionType sourceType) throws ai.tegmentum.wasmtime4j.exception.ValidationException {

        if (!isSubtype(sourceType, targetType)) {
            throw new ai.tegmentum.wasmtime4j.exception.ValidationException(
                String.format("Cannot assign function type %s to %s: not a subtype",
     ...
     */
    public static FunctionType getCommonSupertype(final FunctionType type1, final FunctionType type2) {
