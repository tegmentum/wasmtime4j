 ...
 *
 * <p>CacheLoader provides a way to automatically populate cache entries
 * when they are not present or need to be refreshed. This enables
 * transparent caching where the cache can automatically load missing
 * entries on demand.
 *
 * <p>Example usage:
 *
...
public interface CacheLoader {
     ...
     *
     * <p>This method is called when a cache entry is not present or needs
     * to be refreshed. The implementation should create and return the
     * appropriate resource for the given key and type.
     *
     ...
     *
     * <p>The default implementation calls {@link #load(String, ResourceType)}
     * on a background thread. Implementations can override this to provide
     * truly asynchronous loading.
     *
     * @param key the cache key to load
     * @param type the resource type to load
     ...
     *
     * <p>The default implementation returns true for all types. Implementations
     * can override this to specify which resource types they support.
     *
     ...
     *
     * <p>The default implementation returns the default cache policy.
     * Implementations can override this to specify custom policies
     ...
     *
     * <p>This can be used by the cache to make eviction decisions before
     * actually loading the resource. The default implementation returns -1
     * to indicate that the size is unknown.
     *
     * @param key the cache key
     * @param type the resource type
     ...
     *
     * <p>This allows the loader to perform cleanup operations or
     * update external state when resources are removed from the cache.
     ...
     *
     * <p>This allows the loader to perform error handling, logging, or
     * fallback operations when resource loading fails. The default
     ...
     */
    static <T> CacheLoader of(final ResourceType resourceType,
     ...
     */
    static <T> CacheLoader of(final ResourceType resourceType,
                             final java.util.function.Function<String, T> loadFunction) {
        if (resourceType == null) {
            throw new IllegalArgumentException("Resource type cannot be null");
        }
     ...
     *
     * <p>This can be useful for testing or when automatic loading should
     * not be allowed.
     *
