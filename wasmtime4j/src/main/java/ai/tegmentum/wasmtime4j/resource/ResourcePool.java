 ...
 *
 * <p>ResourcePool manages pools of reusable resources to improve performance and reduce
 * resource allocation overhead. It supports configurable pool sizes, validation, and
 * lifecycle management with automatic cleanup and monitoring.
 *
 * <p>Key features:
 * - Object pooling for expensive-to-create resources
 * - Connection pooling for network and database resources
 * - Configurable pool size limits and overflow handling
 * - Resource validation and health checking
...
public interface ResourcePool extends AutoCloseable {
     ...
     */
    <T> void registerFactory(final ResourceType type, final Supplier<T> factory,
                            final ResourceValidator<T> validator) throws WasmException;
     ...
     */
    <T> void registerFactory(final ResourceType type, final Supplier<T> factory,
                            final PoolConfiguration poolConfig) throws WasmException;

    /**
     * Acquires a resource from the pool.
     *
     * <p>If a valid resource is available in the pool, it is returned immediately.
     * If no resources are available and the pool is below maximum size, a new
     * resource is created. If the pool is at maximum size, this method will block
     * until a resource becomes available or the acquisition timeout is exceeded.
     *
     * @param <T> the resource type
     ...
     * @throws IllegalStateException if no factory is registered for the type
     */
    <T> PooledResource<T> acquire(final ResourceType type, final Duration timeout) throws WasmException;
     ...
     * @throws IllegalStateException if no factory is registered for the type
     */
    <T> java.util.Optional<PooledResource<T>> tryAcquire(final ResourceType type) throws WasmException;
     ...
     *
     * <p>The resource will be validated (if a validator is configured) before
     * being returned to the pool. Invalid resources are discarded instead of
     ...
     *
     * <p>This method should be used when a resource is known to be in an invalid
     * state and should not be returned to the pool for reuse.
     *
     * @param <T> the resource type
     ...
     *
     * <p>This method creates the minimum number of resources for each registered
     * type to ensure the pool is ready for immediate use.
     *
     ...
     *
     * <p>This method runs validation checks on all pooled resources and removes
     ...
     *
     * <p>Configuration changes take effect immediately and may trigger pool
     * resizing or cleanup operations.
     *
     ...
     *
     * <p>Active resources are not affected and will not be returned to the pool
     * when released until the pool is restored.
     *
     ...
     *
     * <p>This method will stop accepting new requests, wait for active resources
     * to be returned, and then close all pooled resources.
     *
