 ...
 *
 * <p>Resources managed by this wrapper are automatically cleaned up when:
 * - The try-with-resources block exits
 * - The resource TTL expires
 * - The resource manager performs cleanup
...
public interface ManagedResource<T> extends AutoCloseable {
     ...
     *
     * <p>This method updates the resource state by checking the underlying resource's
     * validity and updating access timestamps.
     *
     ...
     *
     * <p>This method can be used to keep a resource alive longer than its original TTL.
     * The extension is applied from the current time, not from the original expiration time.
     *
     ...
     *
     * <p>After calling this method, the resource will be eligible for cleanup during
     * the next cleanup cycle, even if it hasn't expired.
     */
    void markForCleanup();
     ...
     *
     * <p>This method can be used to prevent cleanup of a resource that was previously
     * marked for cleanup, as long as the cleanup hasn't started yet.
     *
     ...
     *
     * <p>After calling this method, the resource becomes invalid and cannot be accessed.
     * Any subsequent calls to {@link #get()} will throw IllegalStateException.
     *
     ...
     * It performs the same operation as {@link #release()} but doesn't throw checked exceptions.
     */
    @Override
    void close();
}