 ...
 *
 * <p>ResourceCache provides intelligent caching of frequently used resources to improve performance
 * and reduce resource allocation overhead. It supports configurable eviction policies, cache size
 * limits, and comprehensive monitoring.
 *
 * <p>Key features: - Multiple eviction policies (LRU, LFU, TTL-based, size-based) - Configurable
 * cache size limits and memory thresholds - Thread-safe concurrent access with high performance -
 * Cache statistics and hit/miss tracking - Automatic expiration and cleanup of stale entries -
 * Cache warming and preloading capabilities - Integration with resource lifecycle management
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public interface ResourceCache extends AutoCloseable {
   ...
   */
  <T> void put(
      final String key, final T resource, final ResourceType type, final CachePolicy policy)
   ...
   * @throws IllegalArgumentException if any parameter is null or key is empty
   */
  <T> Optional<T> getAndExtendTtl(
      final String key, final Class<T> expectedType, final Duration ttlExtension)
      throws WasmException;
   ...
   * @throws IllegalArgumentException if any parameter is null or key is empty
   */
  <T> CompletableFuture<T> getOrComputeAsync(
      final String key,
      final ResourceType type,
      final Class<T> expectedType,
      final java.util.function.Supplier<T> supplier);
   ...
   *
   * <p>Configuration changes take effect immediately and may trigger eviction operations if new
   ...
   *
   * <p>This method will stop background operations, persist cache state if configured, and clean up
   * resources.
   *
