 ...
 *
 * <p>PooledResource wraps a resource obtained from a ResourcePool and provides automatic
 * return-to-pool functionality when the resource is no longer needed. It tracks usage statistics
 * and ensures proper pool lifecycle management.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public interface PooledResource<T> extends AutoCloseable {
   ...
   *
   * <p>This should be called when the resource is known to be in an unusable state and should be
   ...
   *
   * <p>After calling this method, the resource becomes unavailable and subsequent calls to {@link
   * #getResource()} will throw IllegalStateException.
   */
  void returnToPool();
