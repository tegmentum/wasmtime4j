 ...
 *
 * <p>ResourceValidator provides a way to check if a pooled resource is still valid and safe to use.
 * Invalid resources are discarded from the pool instead of being returned for reuse.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public interface ResourceValidator<T> {
   ...
   *
   * <p>This method should perform appropriate checks to determine if the resource is in a valid
   * state. The validation should be lightweight as it may be called frequently during pool
   * operations.
   *
