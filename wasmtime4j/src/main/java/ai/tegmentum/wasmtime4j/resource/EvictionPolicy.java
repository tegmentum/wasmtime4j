 ...
 *
 * <p>EvictionPolicy defines the strategy used to remove entries from the cache when space is needed
 * for new entries or when cache limits are exceeded.
 *
 * @since 1.0.0
 */
public enum EvictionPolicy {
   ...
   *
   * <p>Evicts the entries that were accessed least recently. This is the most common eviction
   * policy and works well for most use cases.
   */
   ...
   *
   * <p>Evicts the entries that are accessed least frequently. This works well when access patterns
   * are predictable and some entries are consistently more popular than others.
   */
   ...
   *
   * <p>Evicts the oldest entries first, regardless of access patterns. This is useful when temporal
   * ordering is more important than access frequency.
   */
   ...
   *
   * <p>Evicts entries based on their time-to-live settings. Entries with shorter remaining TTL are
   * evicted first.
   */
   ...
   *
   * <p>Evicts the largest entries first to free up the most memory space. This is useful when
   * memory usage is the primary concern.
   */
   ...
   *
   * <p>Evicts entries with the lowest priority first. This allows explicit control over which
   * entries are more important to retain.
   */
   ...
   *
   * <p>Evicts entries with the highest eviction weight first. This provides fine-grained control
   * over eviction decisions.
   */
   ...
   *
   * <p>Evicts entries randomly. This can be useful for testing or when no particular eviction
   ...
   *
   * <p>Dynamically chooses the best eviction strategy based on current cache usage patterns and
   * performance metrics.
   */
   ...
   * No eviction policy.
   *
   * <p>Never evicts entries automatically. Manual eviction is required when the cache reaches
   * capacity limits.
   */
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Invalid eviction policy: "
              + policyString
              + ". Valid values are: LRU, LFU, FIFO, TTL, SIZE, PRIORITY, WEIGHT, RANDOM, ADAPTIVE,"
              + " NONE",
          e);
    }
  }
