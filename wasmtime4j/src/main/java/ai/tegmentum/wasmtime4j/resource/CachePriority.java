 ...
 *
 * <p>CachePriority provides a way to specify the relative importance of cached resources, affecting
 * their likelihood of being evicted when cache space is limited.
 *
 * @since 1.0.0
 */
public enum CachePriority {
   ...
   *
   * <p>Used for essential resources that are expensive to recreate and are frequently accessed.
   */
   ...
   *
   * <p>Used for frequently accessed resources that are moderately expensive to recreate.
   */
  HIGH("High", 4, 0.3),
   ...
   *
   * <p>Used for regular resources with average access patterns and recreation costs.
   */
  NORMAL("Normal", 3, 1.0),
   ...
   *
   * <p>Used for infrequently accessed resources or resources that are cheap to recreate.
   */
  LOW("Low", 2, 2.0),
   ...
   *
   * <p>Used for temporary or test resources that can be recreated easily.
   */
   ...
   *
   * <p>Lower weights make entries less likely to be evicted. Higher weights make entries more
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Invalid priority string: "
              + priorityString
              + ". Valid values are: CRITICAL, HIGH, NORMAL, LOW, MINIMAL",
          e);
    }
  }
