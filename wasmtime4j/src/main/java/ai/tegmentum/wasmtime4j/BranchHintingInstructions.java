 ...
 *
 * <p>The branch hinting proposal adds annotations to conditional branches to help the runtime
 * engine optimize performance by providing hints about which branch is more likely to be taken.
 * This enables better:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Branch hints are performance optimizations only and do not affect the semantic behavior of
 * WebAssembly programs. Runtimes may ignore hints if they don't provide benefit.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public enum BranchHintingInstructions {
  ...
  /**
   * Hint that a conditional branch is likely to be taken. Applied to br_if instructions to indicate
   * the condition is usually true.
   */
  LIKELY_TAKEN(0x01, "hint.likely", BranchProbability.HIGH, BranchType.CONDITIONAL),

  /**
   * Hint that a conditional branch is unlikely to be taken. Applied to br_if instructions to
   * indicate the condition is usually false.
   */
  UNLIKELY_TAKEN(0x02, "hint.unlikely", BranchProbability.LOW, BranchType.CONDITIONAL),

  /**
   * Hint that a loop is expected to execute many iterations (hot loop). Applied to loop blocks to
   * indicate high iteration count.
   */
  LOOP_HOT(0x03, "hint.loop_hot", BranchProbability.HIGH, BranchType.LOOP),

  /**
   * Hint that a loop is expected to execute few iterations (cold loop). Applied to loop blocks to
   * indicate low iteration count.
   */
  LOOP_COLD(0x04, "hint.loop_cold", BranchProbability.LOW, BranchType.LOOP),

  /**
   * Hint that a branch target is a hot path (frequently executed). Applied to unconditional
   * branches to optimize target code placement.
   */
  HOT_PATH(0x05, "hint.hot_path", BranchProbability.HIGH, BranchType.UNCONDITIONAL),

  /**
   * Hint that a branch target is a cold path (rarely executed). Applied to unconditional branches
   * for error handling or rare cases.
   */
  COLD_PATH(0x06, "hint.cold_path", BranchProbability.LOW, BranchType.UNCONDITIONAL),

  /**
   * Hint for switch/br_table optimization indicating the most likely case. Applied to br_table
   * instructions to optimize dispatch.
   */
  SWITCH_HOT_CASE(0x07, "hint.switch_hot", BranchProbability.HIGH, BranchType.SWITCH),

  /**
   * Hint for function call frequency indicating a hot call site. Applied to call instructions for
   * inlining and optimization decisions.
   */
  CALL_HOT(0x08, "hint.call_hot", BranchProbability.HIGH, BranchType.CALL),

  /**
   * Hint for function call frequency indicating a cold call site. Applied to call instructions for
   * rare or error path calls.
   */
  ...
  BranchHintingInstructions(
      final int opcode,
      final String mnemonic,
      final BranchProbability probability,
   ...
   *
   * <p>This method is used by compilers and optimization tools to attach branch hints to specific
   * instructions or code locations.
   *
  ...
  public static class BranchHintAnnotation {
    ...
    /**
     * Creates a new branch hint annotation.
     *
     * @param hint the branch hinting instruction type
     * @param targetLabel the target label for the hint
     * @param confidence the confidence level (0.0 to 1.0)
     */
    public BranchHintAnnotation(
        final BranchHintingInstructions hint, final String targetLabel, final double confidence) {
      this.hint = hint;
