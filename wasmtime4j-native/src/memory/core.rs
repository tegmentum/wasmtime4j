    ...
    store.with_context_ro(|ctx| {
        ...
        unsafe {
            ...
            if current_value != expected {
                ...
                log::debug!(
                    "Atomic wait32 at offset {} with expected {} and timeout {} ns (simplified)",
                    offset,
                    expected,
                    timeout_nanos
                );
    ...
    store.with_context_ro(|ctx| {
        ...
        unsafe {
            ...
            if current_value != expected {
                ...
                log::debug!(
                    "Atomic wait64 at offset {} with expected {} and timeout {} ns (simplified)",
                    offset,
                    expected,
                    timeout_nanos
                );
...
pub fn force_cleanup_all_handles() -> WasmtimeResult<usize> {
    ...
    log::warn!(
        "Force cleaned {} handles during emergency shutdown",
        total_cleaned
    );
    Ok(total_cleaned)
}
