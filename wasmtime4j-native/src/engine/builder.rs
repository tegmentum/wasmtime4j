...
impl EngineConfigSummary {
    ...
    pub fn to_hash_string(&self) -> String {
        ...
        hasher.update(if self.wasm_function_references {
            b"1"
        } else {
            b"0"
        });
        ...
        hasher.update(if self.wasm_component_model_async {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.wasm_component_model_async_builtins {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.wasm_component_model_async_stackful {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.wasm_component_model_error_context {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.wasm_component_model_gc {
            b"1"
        } else {
            b"0"
        });
        ...
        hasher.update(if self.cranelift_debug_verifier {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.cranelift_nan_canonicalization {
            b"1"
        } else {
            b"0"
        });
        ...
        hasher.update(if self.guard_before_linear_memory {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.memory_init_cow { b"1" } else { b"0" });
        hasher.update(if self.wasm_component_model_threading {
            b"1"
        } else {
            b"0"
        });
        hasher.update(if self.relaxed_simd_deterministic {
            b"1"
        } else {
            b"0"
        });
