...
use crate::memory::{PageSize, PlatformMemoryAllocator, PlatformMemoryConfig};
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_memory_PlatformMemoryManager_nativeCreate(
    ...
    jni_utils::jni_try_ptr(&mut env, || {
        ...
        let allocator =
            PlatformMemoryAllocator::new(config).map_err(|e| WasmtimeError::Runtime {
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        if handle == 0 {
            return Err(WasmtimeError::InvalidParameter {
                message: "Invalid allocator handle".to_string(),
            });
        }

        let allocator = unsafe { &*(handle as *const PlatformMemoryAllocator) };
        let alignment_opt = if alignment == 0 {
            None
        } else {
            Some(alignment as usize)
        };

        let ptr = allocator
            .allocate(size as usize, alignment_opt)
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_memory_PlatformMemoryManager_nativeDeallocate(
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        ...
        let non_null_ptr = std::ptr::NonNull::new(ptr as *mut c_void).ok_or_else(|| {
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_memory_PlatformMemoryManager_nativeGetStats(
    ...
    match (|| -> WasmtimeResult<_> {
        ...
        let stats_class = env
            .find_class("ai/tegmentum/wasmtime4j/jni/memory/PlatformMemoryManager$MemoryStats")
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to find MemoryStats class".to_string(),
                backtrace: None,
            })?;

        let constructor_signature = "(JJJJJJDDJD)V";
        let stats_object = env
            .new_object(
                stats_class,
            ...
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to create MemoryStats object".to_string(),
                backtrace: None,
            })?;
...
>(
    ...
    jni_utils::jni_try_object(&mut env, |env| {
        ...
        let info_class = env
            .find_class("ai/tegmentum/wasmtime4j/jni/memory/PlatformMemoryManager$PlatformInfo")
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to find PlatformInfo class".to_string(),
                backtrace: None,
            })?;
        ...
        let info_object = env
            ...
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to create PlatformInfo object".to_string(),
                backtrace: None,
            })?;
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_memory_PlatformMemoryManager_nativeDetectLeaks(
    ...
    jni_utils::jni_try_object(&mut env, |env| {
        ...
        let leak_class = env
            .find_class("ai/tegmentum/wasmtime4j/jni/memory/PlatformMemoryManager$MemoryLeak")
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to find MemoryLeak class".to_string(),
                backtrace: None,
            })?;

        let leak_array = env
            .new_object_array(leaks.len() as i32, leak_class, JObject::null())
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to create leak array".to_string(),
                backtrace: None,
            })?;
...
>(
    ...
    jni_utils::jni_try_object(&mut env, |env| {
        ...
        let data_len = env
            ...
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to get array length".to_string(),
                backtrace: None,
            })? as usize;
        let mut data_bytes = vec![0i8; data_len];
        env.get_byte_array_region(data, 0, &mut data_bytes)
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to read byte array".to_string(),
                backtrace: None,
            })?;
        ...
        let compressed_array =
            env.new_byte_array(compressed.len() as i32)
                .map_err(|_| WasmtimeError::Runtime {
                    message: "Failed to create compressed array".to_string(),
                    backtrace: None,
                })?;
        let compressed_i8: Vec<i8> = compressed.iter().map(|&x| x as i8).collect();
        env.set_byte_array_region(&compressed_array, 0, &compressed_i8)
            .map_err(|_| WasmtimeError::Runtime {
                message: "Failed to set compressed array".to_string(),
                backtrace: None,
            })?;
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_memory_PlatformMemoryManager_nativeDeduplicate(
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        ...
        let ptr =
            allocator
                .deduplicate_memory(&data_bytes_u8)
                .map_err(|e| WasmtimeError::Runtime {
