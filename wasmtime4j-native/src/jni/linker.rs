...
fn extract_and_clear_java_exception(env: &mut jni::JNIEnv) -> String {
    ...
    let message = match env.call_method(&exception, "getMessage", "()Ljava/lang/String;", &[]) {
        Ok(msg_value) => match msg_value.l() {
            Ok(msg_obj) if !msg_obj.is_null() => match env.get_string((&msg_obj).into()) {
        ...
        Err(_) => {
            ...
            match env.get_object_class(&exception) {
                Ok(cls) => match env.call_method(&cls, "getName", "()Ljava/lang/String;", &[]) {
                    Ok(name_value) => match name_value.l() {
                        Ok(name_obj) if !name_obj.is_null() => {
                            match env.get_string((&name_obj).into()) {
                                Ok(java_str) => {
                                    format!("Exception of type: {}", java_str.to_string_lossy())
                                }
    ...
    fn execute(&self, params: &[WasmValue]) -> WasmtimeResult<Vec<WasmValue>> {
        ...
        let callback_result =
            if self.is_function_reference {
                ...
                let funcref_class = env
                    .find_class("ai/tegmentum/wasmtime4j/jni/JniFunctionReference")
                    .map_err(|e| WasmtimeError::Runtime {
                ...
                let linker_class = env
                    .find_class("ai/tegmentum/wasmtime4j/jni/JniLinker")
                    .map_err(|e| WasmtimeError::Runtime {
        ...
        let result_array = callback_result.l().map_err(|e| WasmtimeError::Runtime {
        ...
        log::info!(
            "JniHostFunctionCallback::execute - Completed successfully with {} results",
            results.len()
        );
        Ok(results)
    }
...
pub fn wasm_values_to_java_array<'local>(
    env: &mut jni::JNIEnv<'local>,
    values: &[WasmValue],
) -> WasmtimeResult<jni::objects::JObject<'local>> {
...
fn java_array_to_wasm_values(
    env: &mut jni::JNIEnv,
    array: &jni::objects::JObject,
) -> WasmtimeResult<Vec<WasmValue>> {
    let array_obj: &jni::objects::JObjectArray =
        unsafe { &*(array as *const jni::objects::JObject as *const jni::objects::JObjectArray) };
    let array_len = env
    ...
    for i in 0..array_len {
        let element =
            env.get_object_array_element(array_obj, i)
                .map_err(|e| WasmtimeError::Runtime {
...
fn java_to_wasm_value(
    env: &mut jni::JNIEnv,
    obj: &jni::objects::JObject,
) -> WasmtimeResult<WasmValue> {
    // Call getValue() to get the boxed value
    let value = env
        .call_method(obj, "getValue", "()Ljava/lang/Object;", &[])
        .map_err(|e| WasmtimeError::Runtime {
    ...
    match type_name.as_str() {
        ...
        "F32" => {
            let float_val = env
                .call_method(&value, "floatValue", "()F", &[])
                .map_err(|e| WasmtimeError::Runtime {
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_JniLinker_nativeInstantiate(
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        ...
        let instance =
            crate::instance::Instance::from_wasmtime_instance(wasmtime_instance, store, module)?;
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_JniLinker_nativeInstantiateNamed(
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        ...
        let wasmtime_instance = linker_lock
            ...
            .map_err(|e| WasmtimeError::Instantiation {
                message: format!(
                    "Failed to instantiate named module '{}': {}",
                    module_name_str, e
                ),
            })?;
        ...
        linker_mut_lock
            .instance(&mut *store_lock, &module_name_str, wasmtime_instance)
        ...
        let instance = crate::instance::Instance::new_without_imports(store, module)?;
...
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_JniLinker_nativeDefineInstance(
    ...
    jni_utils::jni_try_with_default(&mut env, 0, || {
        ...
        let store = unsafe { crate::store::core::get_store_mut(store_handle as *mut c_void)? };
        let instance =
            unsafe { crate::instance::core::get_instance_ref(instance_handle as *const c_void)? };
...
///
/// Returns a String[] where every 3 consecutive elements form a triple:
/// [moduleName, itemName, typeCode] where typeCode is "0"=Func, "1"=Table, "2"=Memory, "3"=Global.
#[no_mangle]
pub extern "system" fn Java_ai_tegmentum_wasmtime4j_jni_JniLinker_nativeLinkerIter(
    mut env: JNIEnv,
    _obj: jobject,
    linker_handle: jlong,
    store_handle: jlong,
