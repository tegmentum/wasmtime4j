...
class WasmLanguageServerTest {
  ...
  void testInitialization() throws ExecutionException, InterruptedException {
    ...
    final TextDocumentClientCapabilities textDocumentCapabilities =
  ...
  void testDocumentOpenAndClose() {
    ...
    final String content =
        "(module (func $add (param $a i32) (param $b i32) (result i32) local.get $a local.get $b"
            + " i32.add))";
  ...
  void testDocumentChange() {
    ...
    final TextDocumentContentChangeEvent change =
        new TextDocumentContentChangeEvent(null, newContent);
    final DidChangeTextDocumentParams changeParams =
        new DidChangeTextDocumentParams(docId, Collections.singletonList(change));
  ...
  void testHoverProvider() throws ExecutionException, InterruptedException {
    ...
    final CompletableFuture<Hover> future =
        languageServer.getTextDocumentService().hover(hoverParams);
  ...
  void testCompletionProvider() throws ExecutionException, InterruptedException {
    ...
    final boolean hasI32 =
        completionList.getItems().stream().anyMatch(item -> "i32".equals(item.getLabel()));
    assertTrue(hasI32);
  }
  ...
  void testDefinitionProvider() throws ExecutionException, InterruptedException {
    ...
    final String uri = "file:///test.wat";
    final String content =
        "(module (func $add (param i32) (result i32)) (export \"add\" (func $add)))";
    ...
    final CompletableFuture<
            Either<java.util.List<? extends Location>, java.util.List<? extends LocationLink>>>
        future = languageServer.getTextDocumentService().definition(definitionParams);
    final Either<java.util.List<? extends Location>, java.util.List<? extends LocationLink>>
        result = future.get();
  ...
  void testCommandExecution() throws ExecutionException, InterruptedException {
    // Arrange
    final String uri = "file:///test.wat";
    final ExecuteCommandParams commandParams =
        new ExecuteCommandParams("wasmtime4j.validate.module", Collections.singletonList(uri));
  ...
  void testConfigurationChange() {
    // Arrange
    final DidChangeConfigurationParams configParams =
        new DidChangeConfigurationParams(
            Collections.singletonMap(
                "wasmtime4j",
                Collections.singletonMap("validation", Collections.singletonMap("enabled", true))));

    // Act & Assert (should not throw)
    assertDoesNotThrow(
        () -> languageServer.getWorkspaceService().didChangeConfiguration(configParams));
  }
  ...
  void testFileWatcher() {
    ...
    final DidChangeWatchedFilesParams watchedFilesParams =
        new DidChangeWatchedFilesParams(Collections.singletonList(changeEvent));
  ...
  void testDocumentSymbols() throws ExecutionException, InterruptedException {
    ...
    final DocumentSymbolParams symbolParams =
  ...
  void testCodeActions() throws ExecutionException, InterruptedException {
    ...
    final Diagnostic diagnostic =
        new Diagnostic(range, "Unclosed parenthesis", DiagnosticSeverity.Error, "wasmtime4j");
    final CodeActionContext context = new CodeActionContext(Collections.singletonList(diagnostic));
    final CodeActionParams actionParams =
        new CodeActionParams(new TextDocumentIdentifier(uri), range, context);
