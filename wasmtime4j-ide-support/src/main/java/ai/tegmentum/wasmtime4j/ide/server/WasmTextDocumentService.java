...
/**
 * Text document service implementation for WebAssembly language server.
 * Handles document-specific operations like completion, hover, diagnostics, etc.
 */
public final class WasmTextDocumentService implements TextDocumentService {
    ...
    public void didSave(final DidSaveTextDocumentParams params) {
        ...
        final TextDocumentItem document = server.getOpenDocuments().get(params.getTextDocument().getUri());
        if (document != null) {
    ...
    public CompletableFuture<Hover> hover(final HoverParams params) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.fine("Hover request for " + uri + " at " + position.getLine() + ":" + position.getCharacter());
    ...
    public CompletableFuture<Either<List<CompletionItem>, CompletionList>> completion(final CompletionParams params) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.fine("Completion request for " + uri + " at " + position.getLine() + ":" + position.getCharacter());
    ...
    public CompletableFuture<Either<List<? extends Location>, List<? extends LocationLink>>> definition(final DefinitionParams params) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.fine("Definition request for " + uri + " at " + position.getLine() + ":" + position.getCharacter());
    ...
    public CompletableFuture<List<? extends Location>> references(final ReferenceParams params) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.fine("References request for " + uri + " at " + position.getLine() + ":" + position.getCharacter());
    ...
    private List<CompletionItem> generateCompletions(final String text, final Position position, final String uri) {
        ...
        items.add(createCompletionItem("f32", CompletionItemKind.Keyword, "32-bit floating point type"));
        items.add(createCompletionItem("f64", CompletionItemKind.Keyword, "64-bit floating point type"));
        ...
        items.add(createCompletionItem("memory", CompletionItemKind.Keyword, "Linear memory declaration"));
        items.add(createCompletionItem("table", CompletionItemKind.Keyword, "Table declaration"));
        items.add(createCompletionItem("global", CompletionItemKind.Keyword, "Global variable declaration"));
        ...
        items.add(createCompletionItem("i32.sub", CompletionItemKind.Function, "Subtract two i32 values"));
        items.add(createCompletionItem("i32.mul", CompletionItemKind.Function, "Multiply two i32 values"));
        items.add(createCompletionItem("i32.div_s", CompletionItemKind.Function, "Signed division of two i32 values"));
        items.add(createCompletionItem("i32.div_u", CompletionItemKind.Function, "Unsigned division of two i32 values"));
        items.add(createCompletionItem("i32.load", CompletionItemKind.Function, "Load i32 from memory"));
        items.add(createCompletionItem("i32.store", CompletionItemKind.Function, "Store i32 to memory"));
    ...
    private CompletionItem createCompletionItem(final String label, final CompletionItemKind kind, final String detail) {
    ...
    private List<Either<SymbolInformation, DocumentSymbol>> extractDocumentSymbols(final TextDocumentItem document) {
    ...
    private List<TextEdit> formatDocument(final TextDocumentItem document, final FormattingOptions options) {
    ...
    private List<TextEdit> formatRange(final TextDocumentItem document, final Range range, final FormattingOptions options) {
        // Format only the specified range
        return Collections.emptyList();
    }

    private List<CodeAction> generateCodeActions(final String uri, final Range range, final CodeActionContext context) {
        ...
        for (final Diagnostic diagnostic : context.getDiagnostics()) {
            if (diagnostic.getMessage().contains("Unclosed parenthesis")) {
                ...
                final List<TextEdit> changes = Arrays.asList(
                    new TextEdit(
