...
/**
 * Main launcher class for the WebAssembly Language Server. This class handles the initialization
 * and lifecycle of the LSP server for WebAssembly development tooling.
 */
public final class WasmLanguageServerLauncher {
   ...
   * @throws Exception if server startup fails
   */
  private static void startStdioServer(final InputStream input, final OutputStream output)
      throws Exception {
    ...
    final Launcher<LanguageClient> launcher =
        Launcher.createLauncher(server, LanguageClient.class, input, output);
