...
/**
 * Test runner for executing WAST-style tests derived from Wasmtime's test suite.
 *
...
public final class WastTestRunner implements AutoCloseable {
   ...
   */
  public WastTestRunner(final RuntimeType runtime) throws Exception {
  ...
  /**
   * Asserts that a binary module fails to compile (malformed).
   *
   * @param wasmBytes the binary module that should fail to compile
   * @param expectedErrorMessage the expected error message (can be null for any error)
   * @throws Exception if the assertion fails
   */
  public void assertMalformedBinary(final byte[] wasmBytes, final String expectedErrorMessage)
      throws Exception {
    try {
      engine.compileModule(wasmBytes);
      throw new AssertionError(
          "Expected malformed binary module to fail compilation but succeeded");
    } catch (final Exception e) {
      // Expected error occurred
      if (expectedErrorMessage != null && !e.getMessage().contains(expectedErrorMessage)) {
        throw new AssertionError(
