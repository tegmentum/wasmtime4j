...
/**
 * Smoke test for the Linker subsystem and host function definition.
 *
 * <p>Validates that a host function can be defined in a Linker, used to instantiate a module with
...
public final class LinkerSmokeTest extends DualRuntimeTest {
  ...
  @ParameterizedTest(name = "linker host function call for {0}")
  @ArgumentsSource(RuntimeProvider.class)
  @DisplayName("host function defined in linker should be callable from WASM")
  void hostFunctionDefinedInLinkerShouldBeCallableFromWasm(final RuntimeType runtime)
      throws Exception {
    setRuntime(runtime);
    ...
    try (final Engine engine = Engine.create();
      ...
      try (final Instance instance = linker.instantiate(store, module)) {
        ...
        LOGGER.info("compute_square(9) = " + results[0].asInt());
      }
    }
  }
}
