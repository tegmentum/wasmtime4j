 ...
 *
 * <p>Provides comprehensive analysis of API documentation coverage and quality,
 * including parity validation between JNI and Panama implementations.
 *
 * @since 1.0.0
 */
public final class DefaultApiDocumentationGenerator implements ApiDocumentationGenerator {

    private static final Logger logger = Logger.getLogger(DefaultApiDocumentationGenerator.class.getName());
     ...
     */
    public DefaultApiDocumentationGenerator(final String sourceRootPath, final List<String> packagePrefixes) {
    ...
    public DocumentationReport generateReport() {
        try {
            ...
            logger.info(String.format("Analysis complete: %d total endpoints, %d documented (%.2f%%)",
                    allEndpoints.size(), documentedEndpoints.size(),
    ...
    public void validateApiParity() {
        try {
            ...
            if (!report.isCompleteParityAchieved()) {
                final String violationSummary = String.format(
                        "API parity validation failed: %d violations detected, %.2f%% compliance",
                        report.getAllViolations().size(),
    ...
    public void validateExamples() {
        try {
            ...
            final List<CodeExample> failedExamples = examples.stream()
                    .filter(example -> !validator.validateExample(example))
                    .collect(Collectors.toList());

            if (!failedExamples.isEmpty()) {
                final String failureSummary = String.format(
            ...
            logger.info(String.format("Example validation successful - all %d examples validated", examples.size()));
    ...
    private List<ApiEndpoint> analyzePackage(final String packagePrefix) throws IOException {
        final List<ApiEndpoint> endpoints = new ArrayList<>();
        final Path packagePath = Paths.get(sourceRootPath, packagePrefix.replace('.', File.separatorChar));
    ...
    private ApiEndpoint createApiEndpoint(final Method method) {
        ...
        final List<String> parameterTypes = Arrays.stream(method.getParameterTypes())
    ...
    private List<ApiEndpoint> filterDocumentedEndpoints(final List<ApiEndpoint> endpoints) {
        return endpoints.stream()
                .filter(ApiEndpoint::isDocumented)
                .collect(Collectors.toList());
    }
    ...
    private CoverageStatistics calculateCoverageStatistics(final List<ApiEndpoint> endpoints) {
        ...
        final int documentedEndpoints = (int) endpoints.stream().filter(ApiEndpoint::isDocumented).count();
        final double coveragePercentage = totalEndpoints > 0 ? (double) documentedEndpoints / totalEndpoints * 100.0 : 0.0;
        ...
        final Map<DocumentationQuality, Integer> qualityDistribution = calculateQualityDistribution(endpoints);
    ...
    private Map<String, Double> calculatePackageCoverage(final List<ApiEndpoint> endpoints) {
        ...
        final Map<String, List<ApiEndpoint>> endpointsByPackage = endpoints.stream()
    ...
    private Map<DocumentationQuality, Integer> calculateQualityDistribution(final List<ApiEndpoint> endpoints) {
        return endpoints.stream()
