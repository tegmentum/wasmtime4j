 ...
 *
 * <p>Provides comprehensive documentation analysis results including coverage statistics, parity
...
public final class DefaultDocumentationReport implements DocumentationReport {
   ...
   */
  public DefaultDocumentationReport(
      final List<ApiEndpoint> documentedEndpoints,
    ...
    this.documentedEndpoints =
        List.copyOf(Objects.requireNonNull(documentedEndpoints, "documentedEndpoints"));
    this.undocumentedEndpoints =
        List.copyOf(Objects.requireNonNull(undocumentedEndpoints, "undocumentedEndpoints"));
    ...
    this.validatedExamples =
        List.copyOf(Objects.requireNonNull(validatedExamples, "validatedExamples"));
  }
  ...
  public String getSummary() {
    ...
    summary.append(
        String.format(
            "Documentation Coverage: %.2f%% (%d/%d endpoints)\n",
    ...
    final long validatedExampleCount =
        validatedExamples.stream().filter(CodeExample::isFullyValidated).count();
    summary.append(
        String.format(
            "Code Examples: %d total, %d validated (%.2f%%)\n",
            validatedExamples.size(),
            validatedExampleCount,
            validatedExamples.isEmpty()
                ? 0.0
                : (double) validatedExampleCount / validatedExamples.size() * 100.0));
    ...
    if (undocumentedEndpoints.isEmpty()) {
      ...
      summary.append(
          String.format(
              "⚠ %d undocumented endpoints require attention\n", undocumentedEndpoints.size()));
    }
  ...
  private double calculateConsistencyScore() {
    ...
    final int excellentCount =
        qualityDistribution.getOrDefault(
  ...
  public String toString() {
    return "DefaultDocumentationReport{"
        ...
        + undocumentedEndpoints.size()
        + ", coveragePercentage="
        + String.format("%.2f%%", coverageStatistics.getOverallCoveragePercentage())
        + ", parityCompliance="
        + String.format("%.2f%%", parityReport.getCompliancePercentage())
        + ", qualityScore="
        + String.format("%.2f", getQualityScore())
        + '}';
  }
}
