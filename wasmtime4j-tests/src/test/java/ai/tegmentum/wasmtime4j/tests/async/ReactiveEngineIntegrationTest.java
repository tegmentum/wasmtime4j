 ...
 *
 * <p>These tests validate the reactive programming patterns, event streams,
 * and Project Reactor integration of the ReactiveEngine implementation.
 *
 * @since 1.0.0
 */
...
class ReactiveEngineIntegrationTest {
  ...
  void setUp() throws Exception {
    ...
    try {
      if (factory.createEngine() instanceof ReactiveEngine) {
        ...
        org.junit.jupiter.api.Assumptions.assumeTrue(false, "ReactiveEngine not available in current runtime");
      }
    } catch (Exception e) {
      org.junit.jupiter.api.Assumptions.assumeTrue(false, "Failed to create ReactiveEngine: " + e.getMessage());
    }
  }
  ...
  void testCompilationEventStream() throws Exception {
    ...
    final Flux<ReactiveEngine.ModuleSource> moduleStream = Flux.just(
    ...
    StepVerifier.create(compilationEvents)
        .expectNextMatches(event -> {
          ...
          assertTrue(event.getProgress() >= 0.0 && event.getProgress() <= 100.0,
  ...
  void testEngineEventStream() throws Exception {
    ...
    final var subscription = eventStream
  ...
  void testFilteredEngineEventStream() throws Exception {
    ...
    final Flux<EngineEvent> errorEventStream = reactiveEngine.getEventStream(
        event -> event.isError()
    );
    ...
    final var subscription = errorEventStream
  ...
  void testPerformanceMetricsStream() throws Exception {
    ...
    final var subscription = metricsStream
        .take(3)
        .subscribe(
            metrics -> {
              assertNotNull(metrics, "Performance metrics should not be null");
              assertTrue(metrics.getAverageCompilationTime() >= 0,
  ...
  void testReactiveEngineStatistics() throws Exception {
    ...
    final Mono<ai.tegmentum.wasmtime4j.EngineStatistics> statsMono = reactiveEngine.getStatisticsReactive();
  ...
  void testHealthStatusMonitoring() throws Exception {
    ...
    StepVerifier.create(healthMono)
        .expectNextMatches(health -> {
          ...
          assertTrue(health.getHealthScore() >= 0.0 && health.getHealthScore() <= 100.0,
  ...
  void testHealthStatusStream() throws Exception {
    ...
    final var subscription = healthStream
        .take(2)
        .subscribe(
            health -> {
  ...
  void testGracefulShutdown() throws Exception {
    ...
    StepVerifier.create(shutdownMono)
        .verifyComplete();
  ...
  void testCompilationEventTypes() throws Exception {
    ...
    final CompilationEvent phaseEvent = CompilationEvent.phaseTransition(
    ...
    final CompilationEvent progressEvent = CompilationEvent.progressUpdate(
    ...
    final CompilationEvent completedEvent = CompilationEvent.completed(
        "test-module", Duration.ofSeconds(2));
    ...
    final CompilationEvent failedEvent = CompilationEvent.failed(
        "test-module", testError, Duration.ofMillis(750));
