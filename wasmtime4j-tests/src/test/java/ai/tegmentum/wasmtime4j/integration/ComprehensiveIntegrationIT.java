...
import ai.tegmentum.wasmtime4j.webassembly.*;
...
/**
 * Comprehensive integration test demonstrating the full testing framework.
 * This test validates the entire integration testing infrastructure.
 */
...
class ComprehensiveIntegrationIT extends BaseIntegrationTest {
    ...
    void shouldDemonstrateCompleteWebAssemblyTestWorkflow() throws IOException {
        ...
        final CrossRuntimeTestRunner.RuntimeTestFunction testFunction = runtime -> {
        ...
        final CrossRuntimeTestResult crossRuntimeResult = CrossRuntimeTestRunner.executeAcrossRuntimes(
            "comprehensive_workflow_test", testFunction);
        ...
        PerformanceRegressionDetector.recordMeasurement("comprehensive_workflow_test", crossRuntimeResult);
        
        final var regressionAnalysis = PerformanceRegressionDetector.detectRegressions(
            "comprehensive_workflow_test", crossRuntimeResult);
        
        LOGGER.info("   - Performance analysis: " + regressionAnalysis.getSummary());
        
        // 4. Test WASI integration
        LOGGER.info("4. Testing WASI integration...");
        final var wasiResult = WasiIntegrationTestRunner.executeWasiTest(
            "comprehensive_wasi_test",
            (runtime, testDir) -> {
                // Simple WASI validation
                return testDir != null ? "wasi_environment_available" : "wasi_environment_unavailable";
            },
            true
        );
        
        LOGGER.info("   - WASI test: " + wasiResult.getSummary());
        
        // 5. Generate comprehensive reports
        LOGGER.info("5. Generating comprehensive reports...");
        
        final CrossRuntimeExecutionSummary executionSummary = CrossRuntimeTestRunner.createExecutionSummary();
        ...
        final var wasiSummary = WasiIntegrationTestRunner.createWasiExecutionSummary();
        ...
        generateComprehensiveReport(stats, executionSummary, wasiSummary, performanceSummary, cacheStats);
    }
    ...
    void shouldValidateCrossPlatformCompatibility() {
        ...
        final CrossRuntimeTestRunner.RuntimeTestFunction platformTest = runtime -> {
        ...
        final var result = CrossRuntimeTestRunner.executeAcrossRuntimes(
            "platform_compatibility_test", platformTest);
    ...
    void shouldCleanupAllTestResourcesProperly() {
        ...
        final CrossRuntimeTestRunner.RuntimeTestFunction cleanupTest = runtime -> {
            try (final var engine = runtime.createEngine();
                 final var store = engine.createStore()) {
     ...
     */
    private void generateComprehensiveReport(final WasmTestSuiteStats stats,
                                           final CrossRuntimeExecutionSummary executionSummary,
                                           final ai.tegmentum.wasmtime4j.wasi.WasiExecutionSummary wasiSummary,
        ...
        report.append("---------------------------\n");
        report.append("Platform: ").append(TestUtils.getOperatingSystem()).append(" (")
              .append(TestUtils.getSystemArchitecture()).append(")\n");
        ...
        report.append("----------------\n");
        report.append(wasiSummary.createReport()).append("\n");
