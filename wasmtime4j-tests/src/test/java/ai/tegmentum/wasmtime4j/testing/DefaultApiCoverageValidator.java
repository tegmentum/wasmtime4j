 ...
 *
 * <p>This validator performs comprehensive analysis of the wasmtime4j API coverage
 * by examining interface definitions, implementation classes, and native method backing.
 */
final class DefaultApiCoverageValidator implements ApiCoverageValidator {

    private static final Logger LOGGER = Logger.getLogger(DefaultApiCoverageValidator.class.getName());

    // Core API interfaces to analyze
    private static final Map<String, Class<?>> CORE_API_INTERFACES = Map.of(
        "Engine", Engine.class,
        "Store", Store.class,
        "Module", Module.class,
    ...
    public CoverageReport validateApiCoverage() {
        ...
        final CoverageReport report = new DefaultCoverageReport(
            totalCoverage,
            coverageByModule,
        ...
        LOGGER.info(String.format("API coverage validation completed in %s. Overall coverage: %.2f%%",
    ...
    public FunctionalityReport validateFunctionality() {
        ...
        for (final Map.Entry<String, ApiCoverageDetail> entry : coverageCache.entrySet()) {
            ...
            if (detail.isImplemented() && detail.hasNativeBacking()) {
                ...
                violations.add(FunctionalityViolation.builder(apiName, "interface")
        ...
        final double overallScore = validatedApis.size() * 100.0 / (validatedApis.size() + failedApis.size());
    ...
    private boolean isMethodImplementedInPanama(final String apiName, final Method method) {
        try {
            ...
            // Check if the method exists and is not abstract
            final Method panamaMethod = panamaClass.getMethod(method.getName(), method.getParameterTypes());
    ...
    private int getTotalMethodCount(final String apiName) {
        ...
        return (int) Arrays.stream(apiInterface.getDeclaredMethods())
            .filter(this::isPublicApiMethod)
            .count();
    }
    ...
    private double calculateTotalCoverage(final Map<String, ApiCoverageDetail> detailedCoverage) {
        ...
        final double totalScore = detailedCoverage.values().stream()
    ...
    private List<String> getPartiallyImplementedApis(final Map<String, ApiCoverageDetail> detailedCoverage) {
        return detailedCoverage.entrySet().stream()
            .filter(entry -> {
                final ApiCoverageDetail detail = entry.getValue();
