 ...
 *
 * <p>These tests verify that error codes are properly aligned between Rust and Java,
 * that error messages are correctly extracted, and that proper exceptions are thrown
 * instead of JVM crashes.
 */
...
class ErrorHandlingTest {
    ...
    void testJniErrorCodeAlignment() {
        ...
        var compilationDesc = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .getErrorCodeDescription(-1);
        assertEquals("Compilation error", compilationDesc);
        
        // Verify validation error (-2) 
        var validationDesc = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .getErrorCodeDescription(-2);
        assertEquals("Validation error", validationDesc);
        
        // Verify runtime error (-3)
        var runtimeDesc = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .getErrorCodeDescription(-3);
        ...
        for (int errorCode = -1; errorCode >= -18; errorCode--) {
            var description = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                .getErrorCodeDescription(errorCode);
            assertFalse(description.startsWith("Unknown error"), 
    ...
    void testPanamaErrorCodeAlignment() {
        ...
        // Verify compilation error (-1)
        var compilationDesc = ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .getErrorDescription(-1);
        assertEquals("Compilation Error", compilationDesc);
        
        // Verify validation error (-2)
        var validationDesc = ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .getErrorDescription(-2);
        assertEquals("Validation Error", validationDesc);
        
        // Verify runtime error (-3)
        var runtimeDesc = ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .getErrorDescription(-3);
        assertEquals("Runtime Error", runtimeDesc);
        
        // Verify all 18 error codes are handled (no "Unknown Error" responses)
        for (int errorCode = -1; errorCode >= -18; errorCode--) {
            var description = ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
    ...
    void testJniExceptionMapping(int errorCode) {
        var exception = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .mapNativeError(errorCode, "Test error message");
    ...
    void testPanamaExceptionMapping(int errorCode) {
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .checkErrorCode(errorCode, "Test operation");
    ...
    void testJniErrorMessageSafety() {
        ...
        var nullException = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .mapNativeError(-1, null);
        assertNotNull(nullException);
        assertNotNull(nullException.getMessage());
        
        // Test empty message handling
        var emptyException = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .mapNativeError(-1, "");
        ...
        var longException = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
            .mapNativeError(-1, longMessage);
    ...
    void testPanamaErrorPointerSafety() {
        ...
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .checkErrorStruct(null, "Test operation", null);
        }, "Should handle null error struct safely");
        
        // Test invalid error codes
        assertThrows(Exception.class, () -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .checkErrorCode(-999, "Invalid error code test");
    ...
    void testErrorRecovery() {
        ...
        assertTrue(ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .isRecoverableError(-7), "Memory errors should be recoverable");
        assertTrue(ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .isRecoverableError(-13), "Invalid parameter errors should be recoverable");
        assertTrue(ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .isRecoverableError(-11), "Resource errors should be recoverable");
        
        // Test non-recoverable errors
        assertFalse(ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .isRecoverableError(-1), "Compilation errors should not be recoverable");
        assertFalse(ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
            .isRecoverableError(-18), "Internal errors should not be recoverable");
    }
    ...
    void testErrorCodeConsistency() {
        ...
        assertEquals("Runtime error", 
            ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper.getErrorCodeDescription(-3));
        assertEquals("Runtime Error", 
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler.getErrorDescription(-3));
        
        // Verify all 18 error codes exist in both implementations
        for (int errorCode = -1; errorCode >= -18; errorCode--) {
            var jniDesc = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                .getErrorCodeDescription(errorCode);
            var panamaDesc = ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .getErrorDescription(errorCode);
            
            assertFalse(jniDesc.startsWith("Unknown error"), 
                "JNI should handle error code " + errorCode);
            assertFalse(panamaDesc.startsWith("Unknown Error"), 
                "Panama should handle error code " + errorCode);
        }
    }
    ...
    void testMemoryLayoutConstants() {
        ...
        assertNotNull(ai.tegmentum.wasmtime4j.panama.MemoryLayouts.WASMTIME_ERROR,
            "WASMTIME_ERROR layout should be defined");
        assertNotNull(ai.tegmentum.wasmtime4j.panama.MemoryLayouts.WASMTIME_ERROR_CODE,
            "WASMTIME_ERROR_CODE varhandle should be defined");
        assertNotNull(ai.tegmentum.wasmtime4j.panama.MemoryLayouts.WASMTIME_ERROR_MESSAGE,
            "WASMTIME_ERROR_MESSAGE varhandle should be defined");
        assertNotNull(ai.tegmentum.wasmtime4j.panama.MemoryLayouts.WASMTIME_ERROR_MESSAGE_LEN,
    ...
    void testJvmCrashPrevention() {
        ...
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                .getErrorCodeDescription(Integer.MIN_VALUE);
        }, "Extreme negative error codes should not crash");
        
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                .getErrorCodeDescription(Integer.MAX_VALUE);
        }, "Extreme positive error codes should not crash");
        
        // Test null/invalid inputs
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                .getSafeErrorMessage(null, null);
        }, "Null error messages should not crash");
        
        assertDoesNotThrow(() -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .createDetailedErrorMessage(null, null, null);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            threads[i] = new Thread(() -> {
                try {
                    ...
                    var exception = ai.tegmentum.wasmtime4j.jni.util.JniExceptionMapper
                        .mapNativeError(errorCode, "Thread " + threadId + " error");
    ...
    void testDefensiveProgramming() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .requireNotEmpty("", "test");
        }, "Empty strings should be rejected");
        
        assertThrows(IllegalArgumentException.class, () -> {
            ai.tegmentum.wasmtime4j.panama.PanamaErrorHandler
                .requireNotEmpty(null, "test");
