 ...
 *
 * <p>These tests validate the complete module serialization pipeline including: - Module
 * serialization with various options - Round-trip serialization and deserialization - Compression
 * support - Streaming serialization for large modules - Metadata extraction and validation - Error
...
class ModuleSerializationIntegrationTest {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ModuleSerializationIntegrationTest.class.getName());
  ...
  void testModuleSerializationWithCompression() throws WasmException {
    ...
    for (final CompressionType compression : CompressionType.values()) {
      final SerializationOptions options =
          baseBuilder.compression(compression).compressionLevel(6).build();
  ...
  void testCompressionEffectiveness() throws WasmException {
    ...
    final SerializationOptions noCompression =
        SerializationOptions.builder().compression(CompressionType.NONE).build();
    final SerializedModule uncompressed = serializer.serialize(largeModule, noCompression);

    // Serialize with GZIP compression
    final SerializationOptions gzipCompression =
    ...
    LOGGER.info(
        String.format(
            "Compression test: uncompressed=%d bytes, compressed=%d bytes, ratio=%.2f%%",
            uncompressedSize,
            compressedSize,
  ...
  void testSerializationWithDebugAndProfilingInfo() throws WasmException {
    ...
    final SerializationOptions basicOptions =
        SerializationOptions.builder().includeDebugInfo(false).includeProfilingInfo(false).build();
    ...
    final SerializationOptions enhancedOptions =
        SerializationOptions.builder().includeDebugInfo(true).includeProfilingInfo(true).build();
    ...
    LOGGER.info(
        String.format(
            "Debug/profiling test: basic=%d bytes, enhanced=%d bytes", basicSize, enhancedSize));
  ...
  void testStreamingSerialization() throws Exception {
    final SerializationOptions options =
        SerializationOptions.builder().compression(CompressionType.GZIP).build();
  ...
  void testSerializationPerformance() throws WasmException {
    ...
    LOGGER.info(
        String.format(
            "Serialization performance: serialization=%dms, deserialization=%dms",
