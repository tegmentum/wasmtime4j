...
public class ComprehensiveNativeFunctionTestSuite extends BaseNativeFunctionTest {
  ...
  void shouldValidateTestInfrastructureSetup() {
    ...
    SUITE_LOGGER.info("Test utilities validated: " + testModules.size() + " test modules available");
  ...
  void shouldExecuteComprehensiveEngineNativeFunctionTests() {
        ...
        testCrossRuntimeCompatibility(
            ...
            runtime -> {
              ...
              try (final var engine = runtime.createEngine()) {
                ...
                // Module compilation
                final var testModules = testUtils.getAllTestModules();
                for (final var testModule : testModules.subList(0, Math.min(3, testModules.size()))) {
    ...
    engineResults.forEach((runtime, result) -> {
              ...
              try (final var engine = runtime.createEngine()) {
                ...
                for (final var testModule : testModules) {
                  try (final var module = engine.compileModule(testModule.getModuleBytes())) {
                    ...
                    SUITE_LOGGER.fine("Expected module operation failure for " + testModule.getName());
                  }
                }
              }
            });
            
    moduleResults.forEach((runtime, result) -> {
      memoryLeakResults.get(runtime).add(result);
      if (result.isLeakDetected()) {
        testsWithMemoryLeaks.incrementAndGet();
  ...
  void shouldExecuteComprehensiveStoreNativeFunctionTests() {
    ...
    storeResults.forEach((runtime, result) -> {
      memoryLeakResults.get(runtime).add(result);
      if (result.isLeakDetected()) {
        testsWithMemoryLeaks.incrementAndGet();
  ...
  void shouldExecuteComprehensiveInstanceNativeFunctionTests() {
    ...
    instanceResults.forEach((runtime, result) -> {
      memoryLeakResults.get(runtime).add(result);
      if (result.isLeakDetected()) {
        testsWithMemoryLeaks.incrementAndGet();
  ...
  void shouldExecuteComprehensiveThreadSafetyTests() {
    ...
    if (!concurrentExecutionResult.isCompleted() || concurrentExecutionResult.getTotalErrors() > 3) {
      threadSafetyFailures.incrementAndGet();
    ...
    SUITE_LOGGER.info("Concurrent creation: " + concurrentCreateResult.getOperationsPerSecond() + " ops/sec");
    SUITE_LOGGER.info("Concurrent execution: " + concurrentExecutionResult.getOperationsPerSecond() + " ops/sec");
  }
  ...
  private static void generateComprehensiveReport() {
    ...
    SUITE_LOGGER.info("  Total Duration: " + totalDuration + "ms (" + 
                      String.format("%.2f", totalDuration / 1000.0) + " seconds)");
    
    SUITE_LOGGER.info("\nTest Performance:");
    testExecutionTimes.forEach((testName, duration) -> {
    ...
    final double leakPercentage = totalLeakTests > 0 ? 
        (testsWithMemoryLeaks.get() * 100.0) / totalLeakTests : 0.0;
    SUITE_LOGGER.info("  Memory Leak Rate: " + String.format("%.2f%%", leakPercentage));
    
    memoryLeakResults.forEach((runtime, results) -> {
      if (!results.isEmpty()) {
        final long leakCount = results.stream().mapToLong(r -> r.isLeakDetected() ? 1 : 0).sum();
        SUITE_LOGGER.info("    " + runtime + ": " + leakCount + "/" + results.size() + " tests with leaks");
      }
    });
    ...
    SUITE_LOGGER.info("  Panama Runtime: " + (TestUtils.isPanamaAvailable() ? "Tested" : "Not Available"));
    ...
    final int expectedTests = expectedTestsPerRuntime * availableRuntimes + 3; // +3 for infrastructure, thread safety, extensive
    ...
    if (testsWithMemoryLeaks.get() == 0 && threadSafetyFailures.get() == 0 && crossRuntimeFailures.get() == 0) {
