...
/**
 * Integration tests for WASI (WebAssembly System Interface) functionality. Tests file system
 * operations, process operations, and WASI compatibility across runtimes.
 */
...
class WasiIntegrationIT extends BaseIntegrationTest {
  ...
  static void setUpWasiTestSuite() throws IOException {
    ...
    final Path wasiTestsDir =
        WasmTestSuiteLoader.getTestSuiteDirectory(WasmTestSuiteLoader.TestSuiteType.WASI_TESTS);
  ...
  void shouldExecuteWasiFileSystemOperationTests() {
    ...
    if (!anySuccessful) {
      LOGGER.warning(
          "No WASI file system tests were successful - this may indicate missing implementation");
    }
  }
  ...
  void shouldExecuteWasiProcessOperationTests() {
    ...
    if (!anySuccessful) {
      LOGGER.warning(
          "No WASI process tests were successful - this may indicate missing implementation");
    }
  }
  ...
  void shouldGenerateComprehensiveWasiExecutionSummary() {
    ...
    WasiIntegrationTestRunner.executeWasiTest(
        "summary_test_1", (runtime, testDir) -> "test1", false);
    WasiIntegrationTestRunner.executeWasiTest(
        "summary_test_2", (runtime, testDir) -> "test2", false);
    WasiIntegrationTestRunner.executeWasiTest(
        "summary_test_3", (runtime, testDir) -> "test3", true);
