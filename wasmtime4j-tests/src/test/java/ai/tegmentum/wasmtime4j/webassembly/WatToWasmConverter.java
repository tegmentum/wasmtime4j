...
/**
 * Utility for converting WebAssembly Text (WAT) format to WebAssembly Binary (WASM) format. Uses
 * the WebAssembly Binary Toolkit (wabt) if available.
 */
public final class WatToWasmConverter {
   ...
   */
  public static boolean convertWatToWasm(final Path watFile, final Path wasmFile)
      throws IOException {
    ...
    if (!isWabtAvailable()) {
      LOGGER.warning(
          "wabt (WebAssembly Binary Toolkit) is not available. "
    ...
    try {
      ...
      if (!finished) {
        ...
        throw new IOException(
            "wat2wasm conversion timed out after " + TIMEOUT_SECONDS + " seconds");
      }
      ...
      if (exitCode != 0) {
        ...
        throw new IOException(
            "wat2wasm conversion failed with exit code " + exitCode + ". Output: " + output);
      }
  ...
  public static boolean isWabtAvailable() {
    try {
      final Process process =
          new ProcessBuilder(WAT2WASM_COMMAND, "--version").redirectErrorStream(true).start();
  ...
  private static void writeWatFile(final Path watFile, final String content) throws IOException {
    Files.writeString(
        watFile, content, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
