...
/**
 * Comparison between two runtime test executions. Provides utilities for comparing results between
...
public final class RuntimeTestComparison {
   ...
   */
  public RuntimeTestComparison(
      final RuntimeTestExecution jniExecution, final RuntimeTestExecution panamaExecution) {
    this.jniExecution = Objects.requireNonNull(jniExecution, "jniExecution cannot be null");
    this.panamaExecution =
        Objects.requireNonNull(panamaExecution, "panamaExecution cannot be null");
  }
  ...
  /**
   * Gets the performance ratio (Panama time / JNI time). Values > 1.0 indicate JNI is faster,
   * values < 1.0 indicate Panama is faster.
   *
  ...
  public String getSummary() {
    ...
    if (bothSuccessful()) {
      ...
      sb.append("  Performance Ratio (Panama/JNI): ")
          .append(String.format("%.2f", getPerformanceRatio()))
          .append('\n');
