...
/**
 * Integration tests for WebAssembly specification compliance.
 * Tests both JNI and Panama implementations against WebAssembly test cases.
 */
...
class WebAssemblySpecIT extends BaseIntegrationTest {
    ...
    void shouldExecuteSimpleArithmeticTestAcrossRuntimes() throws IOException {
        ...
        final RuntimeTestFunction testFunction = runtime -> {
            // Load the add module
        ...
        final CrossRuntimeTestResult result = CrossRuntimeTestRunner.executeAcrossRuntimes(
            "simple_arithmetic_add", testFunction);
    ...
    void shouldValidateRuntimeConsistencyForSimpleOperations() throws IOException {
        ...
        final CrossRuntimeValidationResult validation = CrossRuntimeTestRunner.validateConsistency(
            "runtime_consistency_test", 
            testFunction,
            comparison -> comparison.resultsEqual()
        );
    ...
    void shouldExecuteTestCasesAcrossRuntimes(final WasmTestCase testCase, final RuntimeType runtimeType) {
        ...
        try (final WasmRuntime runtime = createRuntimeForType(runtimeType)) {
            ...
            final WasmModuleValidationResult validation = testDataManager.validateModule(testCase.getModuleBytes());
    ...
    void shouldGenerateExecutionSummaryWithStatistics() {
        ...
        final RuntimeTestFunction simpleTest = runtime -> {
            final byte[] moduleBytes = TestUtils.createSimpleWasmModule();
     ...
     */
    private void validateBasicModuleFunctionality(final Instance instance, final WasmTestCase testCase) {
