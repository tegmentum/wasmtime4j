...
/**
 * Enumeration of WebAssembly test case types for comprehensive specification testing. Categorizes
 * test cases based on their validation purpose and execution requirements.
 *
 * <p>This enumeration provides clear categorization for different types of WebAssembly tests,
 * enabling targeted test execution and comprehensive coverage of WebAssembly specification
 * compliance.
 *
 * @since 1.0.0
 */
public enum TestType {
  ...
  /**
   * Official WebAssembly specification tests. Tests that validate compliance with the WebAssembly
   * specification.
   */
  SPECIFICATION("WebAssembly Specification Tests"),

  /**
   * WebAssembly conformance validation tests. Tests that ensure implementation conformance to
   * WebAssembly standards.
   */
  CONFORMANCE("WebAssembly Conformance Tests"),

  /**
   * Edge case and corner case tests. Tests that validate behavior in unusual or boundary
   * conditions.
   */
  EDGE_CASE("WebAssembly Edge Case Tests"),

  /**
   * Cross-implementation compatibility tests. Tests that validate compatibility across different
   * WebAssembly implementations.
   */
  CROSS_IMPLEMENTATION("Cross-Implementation Compatibility Tests"),

  /**
   * Performance regression tests. Tests that validate performance characteristics and detect
   * regressions.
   */
  PERFORMANCE("WebAssembly Performance Tests"),

  /** Security validation tests. Tests that validate security boundaries and sandboxing. */
  SECURITY("WebAssembly Security Tests"),

  /** Memory management tests. Tests that validate memory allocation, deallocation, and safety. */
  MEMORY("WebAssembly Memory Tests"),

  /**
   * WASI (WebAssembly System Interface) tests. Tests that validate WASI functionality and
   * compliance.
   */
  WASI("WASI Integration Tests"),

  /**
   * Custom wasmtime4j-specific tests. Tests that validate wasmtime4j-specific functionality and
   * features.
   */
  ...
  public boolean isNonFunctionalTest() {
    return this == PERFORMANCE || this == SECURITY || this == MEMORY;
  }
  ...
  public boolean isIntegrationTest() {
    return this == WASI || this == CROSS_IMPLEMENTATION || this == WASMTIME4J_SPECIFIC;
  }

  /**
   * Returns the priority level for this test type. Lower values indicate higher priority.
   *
