 ...
 *
 * <p>This test suite validates that the JNI and Panama implementations of caller context
 * functionality behave identically, ensuring users can switch between runtimes without any
 * functional differences.
 *
 * <p>Tests are designed to run under both runtime environments and compare the behavior and results
 * for consistency.
 */
...
class CrossRuntimeConsistencyTest {
  ...
  void testCallerExportAccessConsistency() {
    ...
    assertEquals(
        jniResults.exportAccessTest.success,
        panamaResults.exportAccessTest.success,
        "Export access success should be identical");
    assertEquals(
        jniResults.exportAccessTest.exportsFound,
        panamaResults.exportAccessTest.exportsFound,
        "Number of exports found should be identical");
    assertEquals(
        jniResults.exportAccessTest.memorySize,
        panamaResults.exportAccessTest.memorySize,
  ...
  void testMultiValueConsistency() {
    ...
    assertEquals(
        jniResults.multiValueTest.success,
        panamaResults.multiValueTest.success,
        "Multi-value success should be identical");
    assertArrayEquals(
        jniResults.multiValueTest.results,
        panamaResults.multiValueTest.results,
  ...
  void testFuelManagementConsistency() {
    ...
    assertEquals(
        jniResults.fuelTest.success,
        panamaResults.fuelTest.success,
        "Fuel operations success should be identical");
    assertEquals(
        jniResults.fuelTest.initialFuel,
        panamaResults.fuelTest.initialFuel,
        "Initial fuel should be identical");
    assertEquals(
        jniResults.fuelTest.finalFuel,
        panamaResults.fuelTest.finalFuel,
        "Final fuel should be identical");
    assertEquals(
        jniResults.fuelTest.fuelAdded,
        panamaResults.fuelTest.fuelAdded,
  ...
  void testEpochDeadlineConsistency() {
    ...
    assertEquals(
        jniResults.epochTest.success,
        panamaResults.epochTest.success,
        "Epoch deadline success should be identical");
    assertEquals(
        jniResults.epochTest.deadlineSet,
        panamaResults.epochTest.deadlineSet,
  ...
  void testPerformanceConsistency() {
    ...
    // Allow up to 3x difference in execution time (generous tolerance for different
    // implementations)
  ...
  void testErrorHandlingConsistency() {
    ...
    assertEquals(
        jniResults.errorTest.success,
        panamaResults.errorTest.success,
        "Error handling success should be identical");
    assertEquals(
        jniResults.errorTest.exceptionsThrown,
        panamaResults.errorTest.exceptionsThrown,
  ...
  private void assumeBothRuntimesAvailable() {
    if (jniResults == null || panamaResults == null) {
      org.junit.jupiter.api.Assumptions.assumeTrue(
          false, "Both JNI and Panama runtimes must be available");
    }
  }
  ...
  private RuntimeTestResults executeTestSuite() throws Exception {
    ...
    Store<TestContext> store =
        Store.builder(engine).withData(new TestContext()).withFuel(1000).build();
  ...
  private void setupConsistencyTestHostFunctions(
      Linker<TestContext> linker, RuntimeTestResults results) {
    ...
    linker.define(
        ...
        HostFunction.multiValueWithCaller(
            (caller, params) -> {
              ...
              return WasmValue.multiValue(WasmValue.i32(sum), WasmValue.i32(product));
            }));
  ...
  private void runExportAccessTest(
  ...
  private void runMultiValueTest(
      Instance instance, Store<TestContext> store, RuntimeTestResults results)
      throws WasmException {
  ...
  private void runFuelOperationsTest(
      Instance instance, Store<TestContext> store, RuntimeTestResults results)
      throws WasmException {
  ...
  private void runEpochDeadlineTest(
      Instance instance, Store<TestContext> store, RuntimeTestResults results) {
