...
/**
 * Engine for executing WebAssembly test cases across different runtime implementations. Provides
 * comprehensive test execution with proper resource management and error handling.
 */
public final class TestExecutionEngine {
  ...
  private TestResult executeCompositeTest(
      final WebAssemblyTestCase testCase, final TestRuntime runtime, final long startTime) {
    final TestResult.Builder resultBuilder =
        TestResult.builder().testCase(testCase).runtime(runtime);
    ...
    try {
      for (final WebAssemblyTestCase subTest : testCase.getSubTests()) {
        ...
        switch (subResult.getStatus()) {
          ...
          case ERROR -> errors++;
          default -> {
            // SKIPPED or other status - no action needed
          }
        }
  ...
  private TestResult executeSingleTest(
    ...
    try {
      ...
      final TestResult result =
          executeWithTimeout(testCase, wasmRuntime, wasmBytes, runtime, startTime);
  ...
  private TestResult executeWithTimeout(
      ...
      final TestRuntime testRuntime,
      final long startTime)
      throws TimeoutException {

    final long timeoutMs = calculateTimeout(testCase);
    final TestResult.Builder resultBuilder =
        TestResult.builder().testCase(testCase).runtime(testRuntime);
  ...
  private TestExecutionResult executeWasm(
  ...
  private TestResult validateTestResult(
      final TestResult result, final WebAssemblyTestCase testCase) {
    final TestExpectedResult expected = testCase.getExpected();
    final TestStatus actualStatus = result.getStatus();

    // Check if the result matches expectations
    final boolean resultMatches =
        switch (expected) {
  ...
  private static final class TestExecutionResult {
    ...
    private TestExecutionResult(
        final boolean success, final String output, final String errorMessage) {
