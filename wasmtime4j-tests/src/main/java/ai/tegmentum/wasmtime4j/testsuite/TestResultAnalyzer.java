...
public final class TestResultAnalyzer {
    ...
    public CrossRuntimeAnalysis performCrossRuntimeAnalysis(final TestExecutionResults results) {
        ...
        for (final TestRuntime runtime : results.getRuntimeResults().keySet()) {
            ...
            final long successCount = runtimeResults.stream()
        ...
        for (final Map.Entry<String, List<TestResult>> entry : resultsByTestCase.entrySet()) {
            ...
            if (testResults.size() > 1) {
                ...
                if (hasSuccesses && hasFailures) {
                    ...
                    discrepancy.append("Test ").append(testId).append(" has different results across runtimes: ");

                    for (final TestResult result : testResults) {
                        discrepancy.append(result.getRuntime().getId())
    ...
    public PerformanceAnalysis analyzePerformance(final TestExecutionResults results) {
        ...
        final double overallAverage = averageExecutionTimes.values().stream()
        ...
        for (final Map.Entry<TestRuntime, Double> entry : averageExecutionTimes.entrySet()) {
            ...
            if (runtimeAvg > overallAverage * 2.0) {
                performanceIssues.add("Runtime " + entry.getKey().getId() +
                    " has significantly higher execution times: " + String.format("%.2f ms", runtimeAvg));
            }
        }
    ...
    public CoverageAnalysis analyzeCoverage(final TestExecutionResults results) {
        ...
        for (final TestCategory category : allCategories) {
            ...
            if (testCount == 0) {
                ...
                coverageGaps.add("Low test coverage for category: " + category.getDisplayName() +
                    " (" + testCount + " tests)");
            }
        }
    ...
    public TestInsights generateInsights(final TestExecutionResults results) {
        ...
        if (runtimeResults.size() > 1) {
            ...
            final double maxPassRate = passRates.values().stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
            final double minPassRate = passRates.values().stream().mapToDouble(Double::doubleValue).min().orElse(0.0);

            if (maxPassRate - minPassRate > 5.0) {
                insights.add("Significant variation in pass rates between runtimes: " +
                    String.format("%.1f%% - %.1f%%", minPassRate, maxPassRate));
                recommendations.add("Investigate runtime-specific failures and improve cross-runtime compatibility");
            }
        }

        // Performance insights
        final long avgExecutionTime = (long) results.getAllResults().stream()
            .mapToLong(TestResult::getExecutionTimeMs)
            .average()
            .orElse(0.0);
