 ...
 *
 * <p>This class integrates the new error handling and diagnostics framework with the existing JNI
 * error mapping system to provide detailed error information.
 *
 * @since 1.0.0
 */
public final class JniErrorHandler {
  ...
  public static WasmError createWasmError(
      final int nativeErrorCode, final long contextPtr, final String operation) {
    ...
    final JniException jniException =
        JniExceptionMapper.mapNativeError(nativeErrorCode, nativeMessage);
  ...
  public static void handleNativeError(
    ...
    throw (JniException)
        wasmError
            .getCause()
            .orElseThrow(() -> new JniException("Failed to extract JNI exception from WasmError"));
  }
   ...
   */
  public static void validateNativeHandle(
      final long handle, final String resourceType, final long contextPtr) throws JniException {
    if (handle == 0) {
  ...
  private static void addEnhancedDiagnostics(
      final WasmError.Builder builder, final long contextPtr, final JniException jniException) {
    try {
  ...
  private static ErrorContext createErrorContext(
      final JniException jniException, final long contextPtr) {
