 ...
 *
 * <p>This class provides JNI-based diagnostics collection with native integration, offering
 * detailed error reporting, performance monitoring, and system health diagnostics specifically
 * optimized for JNI-based WebAssembly operations.
 *
 * <p>Features include:
 *
 * <ul>
...
public final class JniDiagnosticsCollector implements DiagnosticsCollector {
  ...
  public JniDiagnosticsCollector() {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINE,
          "Created JNI diagnostics collector with handle: {0}",
  ...
  public DiagnosticsReport generateReport() {
    ...
    try {
      ...
      final NativeDiagnosticsAnalysis nativeAnalysis =
          nativeGenerateAnalysis(nativeDiagnosticsHandle);
  ...
  public JniHealthCheckResult performHealthCheck() {
    ...
    try {
      final NativeHealthCheckResult nativeResult =
          nativePerformHealthCheck(nativeDiagnosticsHandle);
  ...
  private List<String> generatePerformanceRecommendations(
      final NativeDiagnosticsAnalysis analysis) {
  ...
  private List<String> generateResourceRecommendations(final NativeDiagnosticsAnalysis analysis) {
    ...
    if (analysis.threadContentionRate > 0.1) {
      recommendations.add(
          "High thread contention - consider reducing parallelism or improving synchronization");
    }
  ...
  private static native void nativeRecordEvent(
      long handle,
      int level,
      String category,
      String message,
  ...
  private static native void nativeRecordError(
      long handle, String category, String exceptionType, String message, String stackTrace);

  private static native void nativeRecordPerformanceMetric(
      long handle, String name, double value, String unit);

  private static native void nativeRecordResourceUsage(
      long handle, String resource, long used, long total);
  ...
  public static final class JniHealthCheckResult {
    ...
    public String toString() {
      return String.format(
          "JniHealthCheckResult{overall=%.2f, memory=%.2f, thread=%.2f, jni=%.2f, healthy=%s}",
