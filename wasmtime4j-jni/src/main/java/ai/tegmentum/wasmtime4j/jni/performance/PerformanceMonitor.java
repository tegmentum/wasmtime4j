 ...
 * <ul>
 *   <li>Native call timing and frequency tracking</li>
 *   <li>Memory allocation pattern monitoring</li>
 *   <li>JNI overhead measurement and analysis</li>
 *   <li>Performance regression detection</li>
 *   <li>Real-time performance metrics collection</li>
 *   <li>Detailed profiling hooks for optimization analysis</li>
 * </ul>
 *
 * <p>The monitor tracks all major WebAssembly operations and provides actionable
...
public final class PerformanceMonitor {
    ...
    private static final ConcurrentHashMap<String, OperationStats> OPERATION_STATS = new ConcurrentHashMap<>();
    ...
    public static String getStatistics() {
        ...
        sb.append(String.format("Average JNI overhead: %.0f ns/call %s\n", avgOverhead, 
        ...
        sb.append(String.format("Heap used: %,d / %,d MB\n", 
        ...
        sb.append(String.format("GC time since last check: %d ms (%d collections)\n", 
        ...
        if (OPERATION_STATS.isEmpty()) {
            ...
            for (final OperationStats stats : OPERATION_STATS.values()) {
                sb.append(String.format("%-20s: %,8d calls, avg=%.0fns, min=%.0fns, max=%.0fns, slow=%.1f%%\n",
                    stats.category,
                    stats.getTotalCalls(),
    ...
    public static String getOperationStats(final String category) {
        ...
        return String.format("%s: %,d calls, avg=%.0fns, min=%.0fns, max=%.0fns, slow=%.1f%%",
            category,
            stats.getTotalCalls(),
            stats.getAverageTimeNs(),
            (double) stats.getMinTimeNs(),
    ...
    public static String getPerformanceIssues() {
        ...
        if (avgOverhead > SIMPLE_OPERATION_TARGET_NS) {
            issues.append(String.format("• JNI overhead %.0f ns exceeds target %d ns\n", 
        ...
        if (netAllocated > 100 * 1024 * 1024) { // 100MB
            issues.append(String.format("• Potential memory leak: %,d bytes net allocated\n", netAllocated));
        }

        // Check for excessive slow operations
        for (final OperationStats stats : OPERATION_STATS.values()) {
            if (stats.getSlowOperationRate() > 5.0) { // More than 5% slow operations
                issues.append(String.format("• High slow operation rate for %s: %.1f%%\n", 
