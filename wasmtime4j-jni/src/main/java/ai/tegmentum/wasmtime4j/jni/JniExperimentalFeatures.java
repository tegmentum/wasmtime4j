 ...
 *
 * <p>This class provides access to cutting-edge WebAssembly proposals that are
...
public final class JniExperimentalFeatures {
    ...
    /**
     * Creates a new experimental features configuration with default settings.
     ...
     *
     * <p><strong>WARNING:</strong> This enables all cutting-edge experimental features which
     * may be unstable or cause unexpected behavior.
     *
     ...
     *
     * <p>Stack switching allows WebAssembly modules to suspend and resume execution
     * at specific points, enabling coroutine and fiber implementations.
     *
     ...
     */
    public JniExperimentalFeatures enableStackSwitching(final long stackSize, final int maxConcurrentStacks) {
        if (stackSize < 4096) {
        ...
        logger.info("Stack switching enabled with stack size: " + stackSize +
                   " bytes, max concurrent stacks: " + maxConcurrentStacks);
        return this;
    }
     ...
     *
     * <p>Call/CC allows capturing the current execution state as a continuation
     * that can be invoked later, enabling powerful control flow operations.
     *
     ...
     * @throws RuntimeException if the native operation fails
     */
    public JniExperimentalFeatures enableCallCc(final int maxContinuations, final ContinuationStorageStrategy storageStrategy) {
        ...
        logger.info("Call/CC enabled with max continuations: " + maxContinuations +
     ...
     *
     * <p>Extended constant expressions allow more complex constant computations
     * at compile time, including import-based expressions and global dependencies.
     *
    ...
    public JniExperimentalFeatures enableExtendedConstExpressions(
        ...
        if (!nativeEnableExtendedConstExpressions(nativeConfigPtr, importBasedExpressions,
                                                  globalDependencies, foldingLevel.ordinal())) {
            throw new RuntimeException("Failed to enable extended constant expressions");
        }

        logger.info("Extended constant expressions enabled: import-based=" + importBasedExpressions +
                   ", global-deps=" + globalDependencies + ", folding-level=" + foldingLevel);
        return this;
    }
     ...
     *
     * <p>Flexible vectors allow WebAssembly modules to use dynamic vector sizing
     * and advanced SIMD operations with runtime optimization.
     *
    ...
    public JniExperimentalFeatures enableFlexibleVectors(
            final boolean dynamicSizing,
            final boolean autoVectorization,
            final boolean simdIntegration) {

        if (!nativeEnableFlexibleVectors(nativeConfigPtr, dynamicSizing,
                                        autoVectorization, simdIntegration)) {
            throw new RuntimeException("Failed to enable flexible vectors");
        }

        logger.info("Flexible vectors enabled: dynamic-sizing=" + dynamicSizing +
                   ", auto-vectorization=" + autoVectorization + ", simd-integration=" + simdIntegration);
        return this;
    }
     ...
     *
     * <p>String imports allow WebAssembly modules to import and export strings
     * efficiently with multiple encoding formats and optimization strategies.
     *
    ...
    public JniExperimentalFeatures enableStringImports(
        ...
        if (!nativeEnableStringImports(nativeConfigPtr, encodingFormat.ordinal(),
                                      stringInterning, lazyDecoding, jsInterop)) {
            throw new RuntimeException("Failed to enable string imports");
        }

        logger.info("String imports enabled: encoding=" + encodingFormat +
                   ", interning=" + stringInterning + ", lazy-decoding=" + lazyDecoding +
     ...
     *
     * <p>Resource types provide WebAssembly modules with sophisticated resource
     * management capabilities including automatic cleanup and reference counting.
     *
    ...
    public JniExperimentalFeatures enableResourceTypes(
        ...
        if (!nativeEnableResourceTypes(nativeConfigPtr, automaticCleanup,
                                      referenceCounting, cleanupStrategy.ordinal())) {
            throw new RuntimeException("Failed to enable resource types");
        }

        logger.info("Resource types enabled: auto-cleanup=" + automaticCleanup +
                   ", ref-counting=" + referenceCounting + ", cleanup-strategy=" + cleanupStrategy);
        return this;
    }
     ...
     *
     * <p>Type imports allow WebAssembly modules to import and export types
     * dynamically with configurable validation and resolution mechanisms.
     *
    ...
    public JniExperimentalFeatures enableTypeImports(
        ...
        if (!nativeEnableTypeImports(nativeConfigPtr, validationStrategy.ordinal(),
                                    resolutionMechanism.ordinal(), structuralCompatibility)) {
            throw new RuntimeException("Failed to enable type imports");
        }

        logger.info("Type imports enabled: validation=" + validationStrategy +
                   ", resolution=" + resolutionMechanism + ", structural-compat=" + structuralCompatibility);
        return this;
    }
     ...
     *
     * <p>Shared-everything threads enhance the WebAssembly threads proposal
     * with shared state management and advanced synchronization primitives.
     *
    ...
    public JniExperimentalFeatures enableSharedEverythingThreads(
        ...
        if (!nativeEnableSharedEverythingThreads(nativeConfigPtr, minThreads, maxThreads,
                                                globalStateSharing, atomicOperations)) {
            throw new RuntimeException("Failed to enable shared-everything threads");
        }

        logger.info("Shared-everything threads enabled: min-threads=" + minThreads +
                   ", max-threads=" + maxThreads + ", global-state=" + globalStateSharing +
     ...
     *
     * <p>Custom page sizes allow WebAssembly modules to use non-standard
     * memory page sizes for optimized memory access patterns.
     *
    ...
    public JniExperimentalFeatures enableCustomPageSizes(
            final int pageSize,
            final PageSizeStrategy strategy,
            final boolean strictAlignment) {
        ...
        if (!nativeEnableCustomPageSizes(nativeConfigPtr, pageSize,
                                        strategy.ordinal(), strictAlignment)) {
            throw new RuntimeException("Failed to enable custom page sizes");
        }

        logger.info("Custom page sizes enabled: page-size=" + pageSize +
                   ", strategy=" + strategy + ", strict-alignment=" + strictAlignment);
        return this;
    }
    ...
    /**
     * Disposes of native resources associated with this configuration.
     * After calling this method, the configuration should not be used.
     */
    public void dispose() {
    ...
    private static native boolean nativeEnableStackSwitching(long configPtr, long stackSize, int maxStacks);

    private static native boolean nativeEnableCallCc(long configPtr, int maxContinuations, int storageStrategy);
    ...
    private static native boolean nativeApplyExperimentalFeatures(long experimentalConfigPtr, long wasmtimeConfigPtr);
    ...
    private static native boolean nativeEnableStringImports(
        long configPtr, int encodingFormat, boolean stringInterning, boolean lazyDecoding, boolean jsInterop);
    ...
    private static native boolean nativeEnableTypeImports(
        long configPtr, int validationStrategy, int resolutionMechanism, boolean structuralCompatibility);

    private static native boolean nativeEnableSharedEverythingThreads(
        long configPtr, int minThreads, int maxThreads, boolean globalStateSharing, boolean atomicOperations);
