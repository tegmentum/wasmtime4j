...
public final class WasiAdvancedNetworking {
  ...
  public CompletableFuture<Long> createHttp3ConnectionAsync(
    ...
    LOGGER.info(
        () -> String.format("Creating HTTP/3 connection: host=%s, port=%d", host, port));
  ...
  public CompletableFuture<Http2Response> http2RequestAsync(
    ...
    LOGGER.fine(
        () ->
            String.format(
                "HTTP/2 request: connection=%d, method=%s, path=%s", connectionHandle, method, path));
  ...
  public CompletableFuture<Void> sendWebSocketMessageAsync(
    ...
    if (webSocket.state != WebSocketState.OPEN) {
      throw new WasiException(
          "WebSocket is not open: " + webSocketHandle, WasiErrorCode.ENOTCONN);
    }
    ...
    return CompletableFuture.runAsync(
        () -> {
          try {
            ...
            LOGGER.fine(
                () -> String.format("Sent WebSocket message: handle=%d", webSocketHandle));
          ...
          try {
            final int result =
                nativeEnableUdpMulticast(
                    wasiContext.getNativeHandle(), socketHandle, multicastAddress, interfaceAddress);
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            ...
            throw new RuntimeException("Network interface enumeration failed: " + e.getMessage(), e);
          }
        },
        asyncExecutor);
  }
