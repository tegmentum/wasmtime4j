          ...
          () -> {
            try {
              ...
              final String cacheKey =
                  "engine-" + engine.getNativeHandle() + "-" + config.hashCode();
              resourceCache.put(cacheKey, engine);

              LOGGER.fine(
                  "Created engine with configuration: "
                      + config.getSummary()
                      + ", handle: 0x"
                      + Long.toHexString(engine.getNativeHandle()));
              return engine;
            } catch (final WasmException e) {
              throw new RuntimeException(e);
            } catch (final Exception e) {
              throw new RuntimeException(
                  new WasmException("Unexpected error creating engine with config", e));
            }
          });
    } catch (final RuntimeException e) {
      if (e.getCause() instanceof WasmException) {
