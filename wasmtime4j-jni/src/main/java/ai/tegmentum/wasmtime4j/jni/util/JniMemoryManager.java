 ...
 *
 * <p>This class provides defensive programming by tracking native resource handles and
 * ensuring proper cleanup. It helps prevent native memory leaks and provides debugging
 * information about resource usage.
 *
 * <p>The memory manager is thread-safe and can be used across multiple threads to track
 * and manage native resources.
 */
public final class JniMemoryManager {
    ...
    private static final ConcurrentHashMap<Long, ResourceInfo> ACTIVE_HANDLES = new ConcurrentHashMap<>();
     ...
     */
    public static void registerHandle(final long handle, final String resourceType, final String creationLocation) {
        if (handle == 0) {
            LOGGER.warning("Attempt to register null handle for type: " + resourceType);
            return;
        }

        final ResourceInfo info = new ResourceInfo(resourceType, creationLocation, System.currentTimeMillis());
        final ResourceInfo existing = ACTIVE_HANDLES.put(handle, info);
        
        if (existing != null) {
            LOGGER.warning("Handle " + handle + " was already registered as " + existing.resourceType + 
                    ", now registering as " + resourceType);
        }
    ...
    public static boolean unregisterHandle(final long handle) {
        ...
        if (info != null) {
            ...
            LOGGER.fine("Unregistered native handle " + handle + " for " + info.resourceType + 
    ...
    public static String getMemoryStats() {
        ...
        if (active > 0) {
            ...
            ACTIVE_HANDLES.values().forEach(info -> {
     ...
     *
     * <p>This method should typically be called during application shutdown to detect
     * resource leaks.
     */
    public static void checkForLeaks() {
        ...
        if (activeCount > 0) {
            ...
            ACTIVE_HANDLES.forEach((handle, info) -> {
                final long lifetime = System.currentTimeMillis() - info.creationTime;
                LOGGER.warning("  Leaked handle " + handle + " (" + info.resourceType + 
                        ", age: " + lifetime + "ms" + 
                        (info.creationLocation != null ? ", created at: " + info.creationLocation : "") + ")");
            });
        } else {
     ...
     *
     * <p><strong>WARNING:</strong> This is an emergency cleanup method that should only be
     * used during shutdown. It may cause instability if resources are still in use.
     */
    public static void emergencyCleanup() {
        ...
        final ConcurrentHashMap<Long, ResourceInfo> handlesCopy = new ConcurrentHashMap<>(ACTIVE_HANDLES);
        
        handlesCopy.forEach((handle, info) -> {
            try {
                LOGGER.warning("Emergency cleanup of handle " + handle + " (" + info.resourceType + ")");
                // Note: In a real implementation, you would call the appropriate native cleanup method here
    ...
    private static final class ResourceInfo {
        ...
        ResourceInfo(final String resourceType, final String creationLocation, final long creationTime) {
