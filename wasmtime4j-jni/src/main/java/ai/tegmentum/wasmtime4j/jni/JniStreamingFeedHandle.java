...
/**
 * JNI implementation of StreamingFeedHandle.
 *
 * <p>This class provides manual feeding of WebAssembly bytecode chunks to a streaming compiler
 * through JNI calls to the native Wasmtime library.
 *
 * @since 1.0.0
 */
public final class JniStreamingFeedHandle implements StreamingFeedHandle {

  private static final Logger LOGGER = Logger.getLogger(JniStreamingFeedHandle.class.getName());

  private final JniStreamingCompiler compiler;
  ...
  /**
   * Creates a new JNI streaming feed handle.
   *
   * @param compiler the streaming compiler to feed data to
   */
  JniStreamingFeedHandle(final JniStreamingCompiler compiler) {
    this.compiler = JniValidation.requireNonNull(compiler, "compiler");
    LOGGER.fine("Created JNI streaming feed handle for compiler");
  }
  ...
  public void onBufferAvailable(final Runnable callback) {
    JniValidation.requireNonNull(callback, "callback");
  ...
  public void close() {
    if (closed.compareAndSet(false, true)) {
      LOGGER.fine("Closing JNI streaming feed handle");
