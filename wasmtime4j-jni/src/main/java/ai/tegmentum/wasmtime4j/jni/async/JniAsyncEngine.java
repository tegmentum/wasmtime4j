 ...
 *
 * <p>This implementation provides asynchronous WebAssembly operations using JNI to interface with
 * the native Wasmtime async runtime. All operations return CompletableFuture instances that
 * integrate with Java's async programming model.
 *
...
public final class JniAsyncEngine extends JniEngine implements AsyncEngine {
  ...
  private static native long nativeCompileModuleAsync(
      long engineHandle, byte[] wasmBytes, long operationId);

  private static native long nativeCompileModuleAsyncStream(long engineHandle, long operationId);

  private static native long nativeValidateModuleAsync(
      long engineHandle, byte[] wasmBytes, long operationId);
        ...
        () -> {
          try {
            final long nativeOperationId =
                nativeCompileModuleAsync(getHandle(), wasmBytes, operationId);
    ...
    getAsyncExecutor()
        .execute(
            () -> {
              try {
                final long nativeOperationId =
                    nativeValidateModuleAsync(getHandle(), wasmBytes, operationId);
                if (nativeOperationId == 0) {
                  completeExceptionally(
                      operationId, new WasmException("Failed to start async validation"));
                  return;
                }
        ...
        .execute(
            () -> {
              try {
                ...
                if (nativeOperationId == 0) {
                  completeExceptionally(
                      operationId, new WasmException("Failed to start async store creation"));
                  return;
                }
