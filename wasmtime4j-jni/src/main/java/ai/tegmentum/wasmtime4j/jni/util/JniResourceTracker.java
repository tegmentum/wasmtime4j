 ...
 *
 * <p>This tracker runs in a background thread and provides defensive programming against
 * resource leaks that could cause native memory exhaustion.
 */
public final class JniResourceTracker {
    ...
    private static final ConcurrentHashMap<PhantomReference<?>, CleanupInfo> TRACKED_RESOURCES = new ConcurrentHashMap<>();
     ...
     */
    public static void trackResource(final Object object, final long nativeHandle, 
                                     final String resourceType, final Runnable cleanupAction) {
        if (object == null || nativeHandle == 0 || cleanupAction == null) {
            LOGGER.warning("Invalid parameters for resource tracking: object=" + object + 
                    ", handle=" + nativeHandle + ", action=" + cleanupAction);
            return;
        }
    ...
    public static boolean untrackResource(final Object object) {
        ...
        TRACKED_RESOURCES.entrySet().removeIf(entry -> {
    ...
    public static String getTrackingStats() {
        ...
        if (!TRACKED_RESOURCES.isEmpty()) {
            ...
            TRACKED_RESOURCES.values().forEach(info -> {
     ...
     *
     * <p>This method should be called during application shutdown to cleanly terminate
    ...
    private static void processPhantomReference(final PhantomReference<?> ref) {
        ...
        if (info != null) {
            LOGGER.warning("Detected unreleased native resource: " + info.resourceType + 
                    " with handle " + info.nativeHandle + " - performing emergency cleanup");
            
            try {
                info.cleanupAction.run();
                LOGGER.info("Successfully cleaned up unreleased " + info.resourceType + 
                        " with handle " + info.nativeHandle);
            } catch (final Exception e) {
                LOGGER.severe("Failed to cleanup unreleased " + info.resourceType + 
                        " with handle " + info.nativeHandle + ": " + e.getMessage());
            }
        }
    ...
    private static void performFinalCleanup() {
        ...
        if (resourceCount > 0) {
            ...
            TRACKED_RESOURCES.values().forEach(info -> {
                try {
                    LOGGER.warning("Final cleanup of " + info.resourceType + " with handle " + info.nativeHandle);
                    info.cleanupAction.run();
                } catch (final Exception e) {
                    LOGGER.severe("Failed final cleanup of " + info.resourceType + ": " + e.getMessage());
                }
            });
