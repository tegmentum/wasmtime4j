 ...
 *
 * <p>This benchmark suite measures the performance impact and overhead of enabling experimental
 * features compared to standard WebAssembly execution.
 *
 * <p>The benchmarks cover:
 *
 * <ul>
 *   <li>Configuration creation and setup overhead
 *   <li>Memory usage with experimental features enabled
 *   <li>Runtime performance comparison
 *   <li>Cross-runtime performance (JNI vs Panama)
 * </ul>
 *
 * <p><strong>Note:</strong> Since most experimental features are not yet supported by Wasmtime,
 * these benchmarks primarily measure configuration overhead and setup costs.
 */
...
public class ExperimentalFeaturesPerformanceBenchmark {
  ...
  private static final Logger logger =
      Logger.getLogger(ExperimentalFeaturesPerformanceBenchmark.class.getName());
  ...
  @Benchmark
  public void panamaExtendedConstExpressionsConfiguration(
      PanamaExperimentalState state, Blackhole bh) {
    try {
      PanamaExperimentalFeatures config = state.standardExperimentalConfig;
      config.enableExtendedConstExpressions(
          true, true, PanamaExperimentalFeatures.ConstantFoldingLevel.AGGRESSIVE);
      bh.consume(config.isValid());
    } catch (Exception e) {
      bh.consume(e);
    }
