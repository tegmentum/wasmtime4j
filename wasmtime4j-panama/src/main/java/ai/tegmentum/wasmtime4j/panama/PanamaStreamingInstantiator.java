...
/**
 * Panama implementation of StreamingInstantiator.
 *
 ...
 * @since 1.0.0
 */
public final class PanamaStreamingInstantiator implements StreamingInstantiator {

  private static final Logger LOGGER = Logger.getLogger(PanamaStreamingInstantiator.class.getName());

  private final Module module;
  private final InstantiationConfig config;

  /**
   * Creates a new Panama streaming instantiator.
   *
   * @param module the compiled module ready for instantiation
   * @param config instantiation configuration
   */
  PanamaStreamingInstantiator(final Module module, final InstantiationConfig config) {
    this.module = PanamaValidation.requireNonNull(module, "module");
    this.config = PanamaValidation.requireNonNull(config, "config");
    LOGGER.fine("Created Panama streaming instantiator");
  }
  ...
  public String toString() {
    return "PanamaStreamingInstantiator{"
        + "module=" + module
        + ", config=" + config
        + ", ready=" + isReady()
