 ...
 * 
 * <p>This class provides centralized memory management for all native memory
 * allocations used in Panama FFI operations. It tracks memory usage, provides
 * convenient allocation methods, and ensures proper cleanup through Arena
 * management.</p>
 * 
 * <p>All memory allocated through this manager is associated with the provided
 * Arena and will be automatically freed when the Arena is closed.</p>
 * 
 * @since 1.0.0
 */
public final class PanamaMemoryManager {
    ...
    public MemorySegment allocate(final ValueLayout layout) {
        ...
        try {
            ...
            logger.severe("Failed to allocate memory segment with layout " + layout + ": " + e.getMessage());
            throw e;
        }
    }
     ...
     * 
     * <p>The returned segment contains a copy of the input data.</p>
     * 
    ...
    public MemorySegment[] allocateArray(final int count, final long segmentSize) {
        ...
        try {
            ...
            logger.fine("Allocated array of " + count + " memory segments (" + segmentSize + " bytes each)");
            return segments;
        } catch (Exception e) {
     ...
     * 
     * <p>This method creates a segment that points to memory that is not
     * managed by this manager's arena. The caller is responsible for
     * ensuring the referenced memory remains valid.</p>
     * 
    ...
    public void fill(final MemorySegment segment, final byte value) {
        ...
        try {
            segment.fill(value);
            logger.fine("Filled memory segment with value " + value + " (" + segment.byteSize() + " bytes)");
        } catch (Exception e) {
     ...
     * 
     * <p>This method is a no-op for segments allocated through this manager's
     * arena, as they will be automatically freed when the arena is closed.
     * It only applies to segments created through other means.</p>
     * 
    ...
    public String getMemoryStats() {
        return String.format("PanamaMemoryManager: %d allocations, %d bytes total",
