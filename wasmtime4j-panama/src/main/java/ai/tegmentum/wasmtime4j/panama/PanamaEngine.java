 ...
 * and compiling WebAssembly modules. This implementation uses Panama FFI with
 * optimized method handles and Arena-based resource management for direct native
 * calls to Wasmtime's engine APIs.
 *
 ...
 * and runtime configuration through direct FFI calls.
 */
public final class PanamaEngine implements Engine, AutoCloseable {
    ...
    public PanamaEngine(final ArenaResourceManager resourceManager) throws WasmException {
        ...
        try {
            ...
            this.engineResource = resourceManager.manageNativeResource(
                enginePtr,
                () -> destroyNativeEngineInternal(enginePtr),
                "Wasmtime Engine"
            );
    ...
    public Module compileModule(final byte[] wasmBytes) throws CompilationException, WasmException {
        ...
        try {
            ...
            MemorySegment modulePtr = compileModuleNative(
                engineResource.getNativePointer(), 
                wasmData, 
                wasmBytes.length
            );
     ...
     */
    private MemorySegment compileModuleNative(final MemorySegment enginePtr,
                                              final MemorySegment wasmData,
                                              final long length) throws CompilationException, WasmException {
        ...
        try {
            ...
            int result = nativeFunctions.moduleCompile(
                ...
                wasmData,
                length,
                moduleOutPtr.getSegment()
            );

            // Check for compilation errors using comprehensive error handling
            PanamaErrorHandler.safeCheckError(result, "Module compilation", 
                "WebAssembly module compilation failed");

            // Extract the compiled module pointer
            MemorySegment modulePtr = (MemorySegment) MemoryLayouts.C_POINTER
                .varHandle().get(moduleOutPtr.getSegment(), 0);
