/*
 * Copyright 2025 Tegmentum AI
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package ai.tegmentum.wasmtime4j.panama;

import ai.tegmentum.wasmtime4j.Table;
import ai.tegmentum.wasmtime4j.exception.WasmException;
import ai.tegmentum.wasmtime4j.panama.ffi.WasmtimeBindings;
import ai.tegmentum.wasmtime4j.panama.util.PanamaExceptionMapper;
import ai.tegmentum.wasmtime4j.panama.util.PanamaMemoryManager;
import ai.tegmentum.wasmtime4j.panama.util.PanamaResourceTracker;

import java.lang.foreign.MemorySegment;
import java.util.logging.Logger;

/**
 * Panama FFI implementation of the WebAssembly table interface.
 * 
 * @since 1.0.0
 */
public final class PanamaTable implements Table {
    private static final Logger logger = Logger.getLogger(PanamaTable.class.getName());
    
    private final MemorySegment tableHandle;
    private final WasmtimeBindings bindings;
    private final PanamaMemoryManager memoryManager;
    private final PanamaResourceTracker resourceTracker;
    private final PanamaExceptionMapper exceptionMapper;
    
    private volatile boolean closed = false;

    PanamaTable(final MemorySegment tableHandle,
                final WasmtimeBindings bindings,
                final PanamaMemoryManager memoryManager,
                final PanamaResourceTracker resourceTracker,
                final PanamaExceptionMapper exceptionMapper) throws WasmException {
        this.tableHandle = tableHandle;
        this.bindings = bindings;
        this.memoryManager = memoryManager;
        this.resourceTracker = resourceTracker;
        this.exceptionMapper = exceptionMapper;
        
        try {
            resourceTracker.trackResource(this, tableHandle);
            logger.fine("Created Panama table instance");
        } catch (Exception e) {
            throw exceptionMapper.mapException(e);
        }
    }

    @Override
    public long size() throws WasmException {
        ensureNotClosed();
        
        try {
            // TODO: Implement table size query through FFI
            logger.fine("Getting table size - placeholder implementation");
            return 0L;
        } catch (Exception e) {
            throw exceptionMapper.mapException(e);
        }
    }

    @Override
    public Object get(final long index) throws WasmException {
        ensureNotClosed();
        
        if (index < 0) {
            throw new IllegalArgumentException("Index cannot be negative");
        }

        try {
            // TODO: Implement table element access through FFI
            logger.fine("Getting table element at index " + index + " - placeholder implementation");
            return null;
        } catch (Exception e) {
            throw exceptionMapper.mapException(e);
        }
    }

    @Override
    public void set(final long index, final Object value) throws WasmException {
        ensureNotClosed();
        
        if (index < 0) {
            throw new IllegalArgumentException("Index cannot be negative");
        }

        try {
            // TODO: Implement table element modification through FFI
            logger.fine("Setting table element at index " + index + " - placeholder implementation");
        } catch (Exception e) {
            throw exceptionMapper.mapException(e);
        }
    }

    @Override
    public void close() throws WasmException {
        if (closed) {
            return;
        }

        synchronized (this) {
            if (closed) {
                return;
            }
            
            try {
                resourceTracker.untrackResource(this);
                logger.fine("Closed Panama table instance");
            } catch (Exception e) {
                throw exceptionMapper.mapException(e);
            } finally {
                closed = true;
            }
        }
    }

    public MemorySegment getTableHandle() {
        ensureNotClosed();
        return tableHandle;
    }

    private void ensureNotClosed() {
        if (closed) {
            throw new IllegalStateException("Table has been closed");
        }
    }
}