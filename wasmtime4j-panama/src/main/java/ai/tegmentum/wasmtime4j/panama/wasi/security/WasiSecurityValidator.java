...
/**
 * Security validation and path traversal protection for WASI operations in Panama FFI
 * implementation.
 *
  ...
  private void validateHiddenFile(final Path path) {
    if (!allowHiddenFiles) {
      ...
      if (fileName != null && fileName.toString().startsWith(".")) {
        throw new RuntimeException(String.format("Hidden file access not allowed: %s", path));
      }
    }
  }
  ...
  private void validateSymbolicLinks(final Path path) {
    try {
      if (java.nio.file.Files.isSymbolicLink(path)) {
        throw new RuntimeException(String.format("Symbolic link access not allowed: %s", path));
      }
      ...
      if (strictMode) {
        throw new RuntimeException(String.format("Cannot verify symbolic link status: %s", path));
      }
    }
  }
