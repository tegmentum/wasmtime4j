...
public final class WasiAdvancedNetworkOperations {
    ...
    private static final SymbolLookup SYMBOL_LOOKUP = SymbolLookup.libraryLookup(LIBRARY_NAME, Arena.global());

    // Function layouts
    private static final FunctionDescriptor INIT_DESC = FunctionDescriptor.of(ValueLayout.JAVA_INT);
    private static final FunctionDescriptor WEBSOCKET_CONNECT_DESC = FunctionDescriptor.of(
        ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS);
    private static final FunctionDescriptor WEBSOCKET_SEND_DESC = FunctionDescriptor.of(
        ValueLayout.JAVA_INT, ValueLayout.JAVA_LONG, ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.JAVA_INT);
    private static final FunctionDescriptor HTTP2_CONNECT_DESC = FunctionDescriptor.of(
        ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.ADDRESS);
    private static final FunctionDescriptor GRPC_CONNECT_DESC = FunctionDescriptor.of(
        ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS);
    private static final FunctionDescriptor CLOSE_CONNECTION_DESC = FunctionDescriptor.of(
    ...
    public WasiAdvancedNetworkOperations(final WasiContext wasiContext, final ExecutorService executorService)
        ...
        try {
            ...
            this.http2Connect = LINKER.downcallHandle(
                SYMBOL_LOOKUP.find("http2_connect").orElseThrow(), HTTP2_CONNECT_DESC);
            this.grpcConnect = LINKER.downcallHandle(
                SYMBOL_LOOKUP.find("grpc_connect").orElseThrow(), GRPC_CONNECT_DESC);
            this.closeConnection = LINKER.downcallHandle(
                SYMBOL_LOOKUP.find("advanced_networking_close_connection").orElseThrow(), CLOSE_CONNECTION_DESC);
            this.cleanupAdvancedNetworking = LINKER.downcallHandle(
    ...
    public CompletableFuture<Long> websocketConnect(final String url,
                                                   ...
                                                   final long timeoutMs) throws PanamaException {
    ...
    public CompletableFuture<Void> websocketSend(final long connectionId,
                                                ...
                                                final ByteBuffer data) throws PanamaException {
        ...
        return CompletableFuture.runAsync(() -> {
            try (final Arena callArena = Arena.ofConfined()) {
                ...
                final int result = (int) websocketSend.invokeExact(connectionId, messageType,
                                                                  dataSegment, data.remaining());
                if (result != 0) {
    ...
    public CompletableFuture<Integer> websocketReceive(final long connectionId,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try (final Arena callArena = Arena.ofConfined()) {
                ...
                final int result = (int) websocketReceive.invokeExact(connectionId, bufferSegment,
                                                                     buffer.remaining(), timeoutMs,
                                                                     messageTypeOut, bytesReceivedOut);
                if (result != 0) {
                    if (result == -2) {
