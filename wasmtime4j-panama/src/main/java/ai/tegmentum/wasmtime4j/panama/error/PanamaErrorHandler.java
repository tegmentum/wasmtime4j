 ...
 * the Panama Foreign Function Interface to provide detailed error information.
 *
 * @since 1.0.0
 */
public final class PanamaErrorHandler {
     ...
     */
    public static WasmError createWasmError(final PanamaException panamaException,
     ...
     * @return the comprehensive WebAssembly error
     */
    public static WasmError createWasmError(final int nativeErrorCode,
                                           final Optional<MemorySegment> errorContext,
                                           final String operation) {
        // Create the underlying Panama exception first
        final String nativeMessage = extractNativeErrorMessage(errorContext);
        final PanamaException panamaException = createPanamaException(nativeErrorCode, nativeMessage, operation);
     ...
     */
    public static void handleNativeError(final int nativeErrorCode,
                                        final Optional<MemorySegment> errorContext,
                                        final String operation) throws PanamaException {
        ...
        throw (PanamaException) wasmError.getCause().orElseThrow(
     ...
     */
    public static void validateNativeSegment(final MemorySegment segment, final String resourceType,
                                            final Optional<MemorySegment> errorContext) throws PanamaException {
    ...
    public static int safeReadErrorCode(final Optional<MemorySegment> errorContext) {
        return errorContext.map(segment -> {
            try {
    ...
    private static void addEnhancedDiagnostics(final WasmError.Builder builder,
    ...
    private static ErrorContext createErrorContext(final PanamaException panamaException,
                                                  final Optional<MemorySegment> errorContext) {
        ...
        errorContext.ifPresent(segment -> {
    ...
    private static ErrorDiagnostics createErrorDiagnostics(final PanamaException panamaException,
                                                           final Optional<MemorySegment> errorContext) {
    ...
    private static int extractNativeErrorCode(final PanamaException panamaException,
                                             final Optional<MemorySegment> errorContext) {
    ...
    private static PanamaException createPanamaException(final int nativeErrorCode,
                                                        final String nativeMessage,
                                                        final String operation) {
        final String message = String.format("Panama FFI error in %s: %s (code: %d)",
