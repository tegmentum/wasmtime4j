 ...
 *
 * <p>This class provides comprehensive error handling by mapping native error codes
 * and messages to appropriate Java exceptions. It ensures consistent error reporting
 * across all Panama FFI operations while providing detailed error information.
 *
 * <p>The error handler supports both simple error code mapping and detailed
 * error structure processing for complete error information extraction.
 */
public final class PanamaErrorHandler {
     ...
     * @throws WasmException if the error code indicates failure
     */
    public static void checkErrorCode(final int errorCode, final String customMessage,
                                     final Object... messageArgs) throws WasmException {
        if (errorCode == WASMTIME_SUCCESS) {
            return; // No error
        }
        ...
        LOGGER.log(Level.WARNING, "Native operation failed: " + formattedMessage + " (code=" + errorCode + ")");
        throw exception;
    }
     ...
     */
    public static void checkErrorStruct(final MemorySegment errorStructPtr,
        ...
        try {
            ...
            MemorySegment messagePtr = (MemorySegment) MemoryLayouts.WASMTIME_ERROR_MESSAGE.get(errorStructPtr, 0);
            ...
            if (e instanceof WasmException) {
                ...
                String fallbackMessage = operation != null ? 
                    operation + ": " + DEFAULT_GENERIC_ERROR : DEFAULT_GENERIC_ERROR;
    ...
    public static void safeCheckError(final int errorCode, final String operation, 
                                     final String fallbackMessage) throws WasmException {
        try {
            ...
            if (e instanceof WasmException) {
                ...
                throw new IllegalStateException(fallbackMessage != null ? fallbackMessage : DEFAULT_GENERIC_ERROR, e);
            }
        }
    }
    ...
    private static WasmException createExceptionFromCode(final int errorCode, final String message) {
        return switch (errorCode) {
            ...
            case WASMTIME_ERROR_RUNTIME, WASMTIME_ERROR_TRAP -> new ai.tegmentum.wasmtime4j.exception.RuntimeException(message);
            case WASMTIME_ERROR_MEMORY, WASMTIME_ERROR_INVALID_ARGUMENT, WASMTIME_ERROR_UNSUPPORTED -> 
                new ai.tegmentum.wasmtime4j.exception.RuntimeException(message);
     ...
     */
    private static String extractErrorMessage(final MemorySegment messagePtr, 
    ...
    public static <T> T requireSuccess(final T result, final String operation) {
        if (result == null) {
            String message = operation != null ? 
                operation + ": Operation returned null" : "Operation returned null";
     ...
     */
    public static MemorySegment requireValidPointer(final MemorySegment pointer, 
    ...
    public static int requireValidIndex(final int index, final int size, final String paramName) {
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException(paramName + " out of bounds: " + index + " (size=" + size + ")");
        }
        return index;
    }
     ...
     */
    public static String createDetailedErrorMessage(final String operation,
