...
/**
 * Configurable directory access control system for WASI file operations in Panama FFI context.
 *
...
public final class WasiDirectoryAccessControl {
  ...
  public void grantDirectoryPermissions(
      final String directoryPath, final Set<WasiFileOperation> permissions, final boolean recursive)
  ...
  private Path normalizePath(final Path path) throws PanamaException {
    try {
      ...
      throw new PanamaException("Invalid path: " + path, e);
    }
  }
    ...
    if (strictPathValidation) {
      try {
        ...
        if (Files.exists(path) && !Files.isDirectory(path)) {
          throw new PanamaException("Path is not a directory: " + path);
        }
      } catch (final SecurityException e) {
        LOGGER.warning(String.format("Cannot verify directory status: %s", e.getMessage()));
        if (strictPathValidation) {
          throw new PanamaException("Cannot verify directory: " + path, e);
        }
      }
    }
  }
  ...
  public static final class Builder {
    ...
    public Builder withGlobalDefaultPermission(final WasiFileOperation permission) {
      PanamaValidation.requireNonNull(permission, "permission");
