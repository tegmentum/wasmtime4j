...
public final class PanamaExperimentalFeatures implements AutoCloseable {
    ...
    static {
        try {
            ...
            createAllConfig = nativeLibrary
                .find("wasmtime4j_experimental_create_all_config")
                .map(addr -> Linker.nativeLinker().downcallHandle(addr,
                    FunctionDescriptor.of(ValueLayout.ADDRESS)))
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_create_all_config"));

            enableStackSwitching = nativeLibrary
                .find("wasmtime4j_experimental_enable_stack_switching")
                .map(addr -> Linker.nativeLinker().downcallHandle(addr,
                    FunctionDescriptor.of(ValueLayout.JAVA_INT,
                        ValueLayout.ADDRESS, ValueLayout.JAVA_LONG, ValueLayout.JAVA_INT)))
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_enable_stack_switching"));

            enableCallCc = nativeLibrary
                .find("wasmtime4j_experimental_enable_call_cc")
                .map(addr -> Linker.nativeLinker().downcallHandle(addr,
                    FunctionDescriptor.of(ValueLayout.JAVA_INT,
                        ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT)))
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_enable_call_cc"));

            enableExtendedConstExpressions = nativeLibrary
                ...
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_enable_extended_const_expressions"));

            applyFeatures = nativeLibrary
                .find("wasmtime4j_experimental_apply_features")
                .map(addr -> Linker.nativeLinker().downcallHandle(addr,
                    FunctionDescriptor.of(ValueLayout.JAVA_INT,
                        ValueLayout.ADDRESS, ValueLayout.ADDRESS)))
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_apply_features"));

            destroyConfig = nativeLibrary
                .find("wasmtime4j_experimental_destroy_config")
                .map(addr -> Linker.nativeLinker().downcallHandle(addr,
                    FunctionDescriptor.ofVoid(ValueLayout.ADDRESS)))
                .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_experimental_destroy_config"));

        } catch (final Exception e) {
            logger.severe("Failed to initialize Panama experimental features bindings: " + e.getMessage());
     ...
     */
    public PanamaExperimentalFeatures enableStackSwitching(final long stackSize, final int maxConcurrentStacks) {
        if (stackSize < 4096) {
        ...
        try {
            final int result = (int) enableStackSwitching.invokeExact(nativeConfigPtr, stackSize, maxConcurrentStacks);
            if (result == 0) {
     ...
     * @throws RuntimeException if the native operation fails
     */
    public PanamaExperimentalFeatures enableCallCc(final int maxContinuations, final ContinuationStorageStrategy storageStrategy) {
        ...
        try {
            final int result = (int) enableCallCc.invokeExact(nativeConfigPtr, maxContinuations, storageStrategy.ordinal());
            if (result == 0) {
    ...
    public PanamaExperimentalFeatures enableExtendedConstExpressions(
        ...
        try {
            final int result = (int) enableExtendedConstExpressions.invokeExact(
                nativeConfigPtr,
