 ...
 *
 * <p>A Store is an execution context that holds the runtime state for WebAssembly instances.
 * This implementation uses Panama FFI with Arena-based resource lifecycle management for
 * optimal performance and automatic cleanup.
 *
 * <p>Each store maintains isolated linear memory, globals, and execution state through
 * direct native calls. Objects like instances, functions, and memories are tied to a
 * specific store and managed through the Arena resource system.
 */
public final class PanamaStore implements Store, AutoCloseable {
    ...
    public PanamaStore(final PanamaEngine engine) throws WasmException {
        ...
        try {
            ...
            this.storeResource = resourceManager.manageNativeResource(
                storePtr,
                () -> destroyNativeStoreInternal(storePtr),
                "Wasmtime Store"
            );
    ...
    public void setData(final Object data) {
        ensureNotClosed();
        userData.set(data);
        LOGGER.finest("Set store user data: " + (data != null ? data.getClass().getSimpleName() : "null"));
    }
    ...
    public PanamaInstance instantiateModule(final PanamaModule module) throws WasmException {
        ...
        try {
            ...
            return module.createInstance(storeResource.getNativePointer(), java.util.Collections.emptyList());
            
        } catch (Exception e) {
            String detailedMessage = PanamaErrorHandler.createDetailedErrorMessage(
                "Module instantiation", 
                "module=" + module.getModulePointer(), 
                e.getMessage()
            );
            throw new WasmException(detailedMessage, e);
        }
