...
public final class PanamaStore implements Store, AutoCloseable {
  ...
  public void setFuel(final long fuel) throws WasmException {
    ...
    try {
      ...
      PanamaErrorHandler.safeCheckError(
          result, "Fuel setting", "Failed to set fuel to " + fuel);

      LOGGER.fine("Successfully set fuel to: " + fuel);
  ...
  public long getFuel() throws WasmException {
    ...
    try {
      ...
      int result = nativeFunctions.storeGetFuelRemaining(
          storeResource.getNativePointer(), fuelPtr.getSegment());
      PanamaErrorHandler.safeCheckError(
          result, "Fuel retrieval", "Failed to get remaining fuel");

      // Extract the fuel value
      long fuel = (Long) MemoryLayouts.C_SIZE_T.varHandle().get(fuelPtr.getSegment(), 0);

      LOGGER.fine("Retrieved fuel: " + fuel);
      return fuel;
  ...
  public void addFuel(final long fuel) throws WasmException {
    ...
    try {
      ...
      int result = nativeFunctions.storeAddFuel(storeResource.getNativePointer(), fuel);
      PanamaErrorHandler.safeCheckError(
          result, "Fuel addition", "Failed to add fuel: " + fuel);

      LOGGER.fine("Successfully added fuel: " + fuel);
  ...
  public void setEpochDeadline(final long ticks) throws WasmException {
    ...
    try {
      ...
      PanamaErrorHandler.safeCheckError(
          result, "Epoch deadline setting", "Failed to set epoch deadline to " + ticks);
