...
public final class WasiFileSystem {
  ...
  public long seekFile(final int fileDescriptor, final long position, final int whence) {
    ...
    try {
      final long newPosition = switch (whence) {
        case 0 -> fileChannel.position(position).position(); // SEEK_SET
        case 1 -> fileChannel.position(fileChannel.position() + position).position(); // SEEK_CUR
        case 2 -> fileChannel.position(fileChannel.size() + position).position(); // SEEK_END
        default -> throw new WasiFileSystemException("Invalid seek whence: " + whence, "EINVAL");
      };
