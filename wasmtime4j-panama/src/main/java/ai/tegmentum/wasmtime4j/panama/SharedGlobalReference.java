 ...
 *
 * <p>This class provides a lightweight reference to a global that can be passed between
 * different WebAssembly modules while maintaining type safety and mutability constraints.
 * It uses weak references to avoid preventing garbage collection of the underlying global.
 *
 * <p>Shared references are thread-safe and can be used concurrently from multiple modules.
 * They provide the same interface as regular globals but delegate operations to the
 * underlying shared global.
 *
 * @since 1.0.0
 */
public final class SharedGlobalReference implements WasmGlobal, AutoCloseable {
   ...
   */
  SharedGlobalReference(final PanamaGlobal global, final int wasmType, final boolean mutable, 
                       final WasmValue initialValue, final ArenaResourceManager resourceManager) {
  ...
  public WasmValue get() {
    ...
    if (global == null || global.isClosed()) {
      throw new IllegalStateException("Shared global reference is no longer valid - original global was garbage collected or closed");
    }
  ...
  /**
   * Gets the underlying global if still available.
   * This should be used carefully as it breaks the weak reference pattern.
   *
  ...
  public String toString() {
    ...
    return String.format("SharedGlobalReference{type=%s, mutable=%s, valid=%s}", 
        typeName, mutable, valid);
  }
