...
public final class PanamaFunction implements WasmFunction, TypedFunc.TypedFunctionSupport {
  ...
  public WasmValue[] call(final WasmValue... params) throws WasmException {
    ...
    // Lightweight closed check — PanamaInstance's own lock guards the native pointer.
    // PanamaFunction holds no native resources, so a full RWLock is unnecessary.
  ...
  public java.util.concurrent.CompletableFuture<WasmValue[]> callAsync(final WasmValue... params) {
    final boolean useNativeAsync = instance.isAsync();
    return java.util.concurrent.CompletableFuture.supplyAsync(
        () -> {
          try {
            if (useNativeAsync) {
              return instance.callFunctionAsync(name, params);
            }
            return call(params);
