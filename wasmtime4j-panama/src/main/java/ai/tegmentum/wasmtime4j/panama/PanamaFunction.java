 ...
 * 
 * <p>WebAssembly functions provide callable interfaces between host code and WebAssembly
 * modules. This implementation uses Panama FFI with Stream 1 & 2 infrastructure for
 * type-safe function invocation through optimized method handles with zero-copy
 * parameter and return value marshalling.</p>
 * 
 * <p>Function invocation includes comprehensive parameter validation, type checking,
 * and automatic conversion between Java and WebAssembly value types. All operations
 * use Arena-based resource management for automatic cleanup.</p>
 * 
 * @since 1.0.0
 */
public final class PanamaFunction implements Function, AutoCloseable {
     ...
     */
    public PanamaFunction(final MemorySegment functionPtr,
                          final ArenaResourceManager resourceManager,
                          final PanamaInstance parentInstance) throws WasmException {
        ...
        try {
            ...
            this.functionResource = resourceManager.manageNativeResource(
                functionPtr,
                () -> destroyNativeFunctionInternal(functionPtr),
                "Wasmtime Function"
            );
        ...
        try {
            ...
            if (expectedParams > 0) {
                ...
                for (int i = 0; i < expectedParams; i++) {
                    MemorySegment paramSlot = paramsArray.asSlice(i * MemoryLayouts.WASM_VAL.byteSize(), 
            ...
            boolean success = invokeFunctionNative(paramsArray, expectedParams, resultsArray, expectedResults);
            ...
            for (int i = 0; i < expectedResults; i++) {
                MemorySegment resultSlot = resultsArray.asSlice(i * MemoryLayouts.WASM_VAL.byteSize(), 
                                                              MemoryLayouts.WASM_VAL.byteSize());
            ...
            String detailedMessage = PanamaErrorHandler.createDetailedErrorMessage(
                "Function invocation", 
                "function=" + functionResource.getNativePointer() + 
    ...
    private void initializeFunctionType() throws WasmException {
        try {
            ...
            LOGGER.fine("Initialized function type: params=" + parameterTypes.size() + 
    ...
    private MemorySegment getFunctionType() throws Exception {
        ...
        MethodHandle funcType = nativeFunctions.getFunction(
            "wasmtime_func_type",
     ...
     */
    private List<Integer> extractParameterTypes(final MemorySegment functionTypePtr) throws Exception {
     ...
     */
    private boolean invokeFunctionNative(final MemorySegment paramsArray, final int paramCount,
                                        final MemorySegment resultsArray, final int resultCount) throws Exception {
        // Call wasmtime_func_call through cached method handle
        MethodHandle funcCall = nativeFunctions.getFunction(
            "wasmtime_func_call",
            FunctionDescriptor.of(
        ...
        return (boolean) funcCall.invoke(
     ...
     */
    private void marshalParameter(final Object javaValue, final int wasmType, final MemorySegment valueSlot) {
    ...
    private int convertToI32(final Object value) {
        if (value instanceof Integer) {
            ...
            throw new IllegalArgumentException("Cannot convert " + value.getClass().getSimpleName() + " to i32");
        }
    }
    ...
    private long convertToI64(final Object value) {
        if (value instanceof Long) {
            ...
            throw new IllegalArgumentException("Cannot convert " + value.getClass().getSimpleName() + " to i64");
        }
    }
    ...
    private float convertToF32(final Object value) {
        if (value instanceof Float) {
            ...
            throw new IllegalArgumentException("Cannot convert " + value.getClass().getSimpleName() + " to f32");
        }
    }
    ...
    private double convertToF64(final Object value) {
        if (value instanceof Double) {
            ...
            throw new IllegalArgumentException("Cannot convert " + value.getClass().getSimpleName() + " to f64");
        }
    }
