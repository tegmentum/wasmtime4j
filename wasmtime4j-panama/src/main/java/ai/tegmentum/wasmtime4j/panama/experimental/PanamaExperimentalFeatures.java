...
import ai.tegmentum.wasmtime4j.experimental.ProfilingGranularity;
import ai.tegmentum.wasmtime4j.experimental.SecurityLevel;
import ai.tegmentum.wasmtime4j.experimental.TierUpStrategy;
 ...
 *
 * <p><b>WARNING:</b> This class provides access to highly experimental features that are
 * unstable and subject to change. Use only for testing, development, and research.
 *
...
public final class PanamaExperimentalFeatures implements AutoCloseable {
    ...
    // Function signatures
    private static final FunctionDescriptor CREATE_FEATURES_DESC = FunctionDescriptor.of(ValueLayout.ADDRESS);
    private static final FunctionDescriptor ENABLE_FEATURE_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.ADDRESS);
    private static final FunctionDescriptor DISABLE_FEATURE_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.ADDRESS);
    private static final FunctionDescriptor CONFIGURE_STACK_SWITCHING_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.JAVA_LONG, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT);
    private static final FunctionDescriptor CONFIGURE_CALL_CC_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT);
    private static final FunctionDescriptor CONFIGURE_SECURITY_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT);
    private static final FunctionDescriptor CONFIGURE_PROFILING_DESC = FunctionDescriptor.ofVoid(
        ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_LONG);
    private static final FunctionDescriptor START_PROFILING_DESC = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS);
    private static final FunctionDescriptor STOP_PROFILING_DESC = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS);
    private static final FunctionDescriptor GET_RESULTS_DESC = FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS);
    ...
    static {
        try {
            ...
            CREATE_EXPERIMENTAL_FEATURES = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_create_experimental_features").orElseThrow(),
                CREATE_FEATURES_DESC);

            ENABLE_EXPERIMENTAL_FEATURE = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_enable_experimental_feature").orElseThrow(),
                ENABLE_FEATURE_DESC);

            DISABLE_EXPERIMENTAL_FEATURE = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_disable_experimental_feature").orElseThrow(),
                DISABLE_FEATURE_DESC);

            CONFIGURE_STACK_SWITCHING = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_configure_stack_switching").orElseThrow(),
                CONFIGURE_STACK_SWITCHING_DESC);

            CONFIGURE_CALL_CC = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_configure_call_cc").orElseThrow(),
                CONFIGURE_CALL_CC_DESC);

            CONFIGURE_ADVANCED_SECURITY = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_configure_advanced_security").orElseThrow(),
                CONFIGURE_SECURITY_DESC);

            CONFIGURE_ADVANCED_PROFILING = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_configure_advanced_profiling").orElseThrow(),
                CONFIGURE_PROFILING_DESC);

            START_PROFILING = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_start_profiling").orElseThrow(),
                START_PROFILING_DESC);

            STOP_PROFILING = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_stop_profiling").orElseThrow(),
                STOP_PROFILING_DESC);

            GET_PROFILING_RESULTS = NATIVE_LINKER.downcallHandle(
                NATIVE_LOOKUP.find("wasmtime4j_get_profiling_results").orElseThrow(),
                GET_RESULTS_DESC);

            DESTROY_EXPERIMENTAL_FEATURES = NATIVE_LINKER.downcallHandle(
    ...
    private void applyConfiguration(final ExperimentalFeatureConfig config) {
        ...
        try {
            ...
            LOGGER.info("Applied experimental features configuration with {} enabled features",
    ...
    public void enableExperimentalFeature(final ExperimentalFeature feature) {
        ...
        try {
            final MemorySegment featureKeySegment = arena.allocateUtf8String(feature.getKey());
     ...
     */
    public void configureStackSwitching(final long stackSize, final int maxStacks, final int strategy) {
        ensureNotClosed();
        ...
        try {
            ...
            LOGGER.info("Configured stack switching: stackSize={}, maxStacks={}, strategy={}",
                       stackSize, maxStacks, strategy);
        } catch (final Throwable e) {
     ...
     */
    public void configureCallCc(final int maxContinuations, final int storageStrategy, final int compressionEnabled) {
        ensureNotClosed();
        ...
        try {
            ...
            LOGGER.info("Configured call/cc: maxContinuations={}, storageStrategy={}, compression={}",
                       maxContinuations, storageStrategy, compressionEnabled != 0);
        } catch (final Throwable e) {
     ...
     */
    public void configureAdvancedSecurity(final int securityLevel, final int enableSandboxing,
                                         final int enableResourceLimits, final int maxMemoryMb) {
        ensureNotClosed();

        try {
            CONFIGURE_ADVANCED_SECURITY.invoke(nativeHandle, securityLevel, enableSandboxing,
                                             enableResourceLimits, maxMemoryMb);
            LOGGER.info("Configured advanced security: level={}, sandboxing={}, resourceLimits={}, memory={}MB",
                       securityLevel, enableSandboxing != 0, enableResourceLimits != 0, maxMemoryMb);
        } catch (final Throwable e) {
     ...
     */
    public void configureAdvancedProfiling(final int enablePerfCounters, final int enableTracing,
                                          final int granularity, final long samplingInterval) {
        ensureNotClosed();

        try {
            CONFIGURE_ADVANCED_PROFILING.invoke(nativeHandle, enablePerfCounters, enableTracing,
                                               granularity, samplingInterval);
            LOGGER.info("Configured advanced profiling: perfCounters={}, tracing={}, granularity={}, interval={}μs",
                       enablePerfCounters != 0, enableTracing != 0, granularity, samplingInterval);
        } catch (final Throwable e) {
    ...
    public String getProfilingResults() {
        ...
        try {
            final MemorySegment resultSegment = (MemorySegment) GET_PROFILING_RESULTS.invoke(nativeHandle);
            if (resultSegment == null || resultSegment.address() == 0) {
                return "No profiling results available";
            }

            final String results = resultSegment.getUtf8String(0);
