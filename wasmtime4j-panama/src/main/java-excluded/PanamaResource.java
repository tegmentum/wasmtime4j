...
public abstract class PanamaResource implements AutoCloseable {
  ...
  private static final class ResourceCleanup {
    ...
    ResourceCleanup(
        final MemorySegment nativeHandle, final String resourceType, final PanamaResourceCleanup cleanup) {
   ...
   */
  public final MemorySegment getNativeHandle() {
    ensureNotClosed();
    return nativeHandle;
  }
  ...
  public final void close() {
    if (closed.compareAndSet(false, true)) {
      try {
        doClose();
        LOGGER.fine(String.format("Closed %s resource with handle: %s", getResourceType(), nativeHandle));
  ...
  /**
   * Performs the actual resource cleanup. Subclasses must implement this method to handle
  ...
  private void doCleanupSafely(final MemorySegment nativeHandle) {
    try {
      ...
      LOGGER.warning(
          String.format(
              "Error during phantom cleanup of %s resource: %s", getResourceType(), e.getMessage()));
    }
  }
      ...
      try {
        ...
        if (cleanup != null) {
          try {
            ...
            LOGGER.fine(
                String.format(
                    "Phantom cleanup completed for %s resource", cleanup.resourceType));
          } catch (final Exception e) {
            LOGGER.warning(
                String.format(
