 ...
 *
 * <p>This class provides comprehensive configuration options for WebAssembly execution, including
 * compilation strategies, optimization levels, resource limits, and performance tuning parameters
 * specifically optimized for Panama FFI-based operations.
 *
 * <p>Features include:
 *
 * <ul>
 *   <li>Compilation strategy configuration (eager, lazy, streaming)
 *   <li>Optimization level control (none, speed, size)
 *   <li>Resource limits and quotas
 *   <li>Memory management configuration
 *   <li>Thread pool and concurrency settings
 *   <li>Debug and profiling options
 *   <li>WASM feature enablement
 * </ul>
 *
 * @since 1.0.0
 */
public final class PanamaAdvancedConfiguration implements AdvancedConfiguration {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PanamaAdvancedConfiguration.class.getName());
  ...
  static {
    try {
      ...
      DESTROY_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_destroy")
      ...
      APPLY_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_apply")
              .map(
                  addr -> linker.downcallHandle(addr, FunctionDescriptor.of(JAVA_BOOLEAN, ADDRESS)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_apply"));

      RESET_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_reset")
              .map(addr -> linker.downcallHandle(addr, FunctionDescriptor.ofVoid(ADDRESS)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_reset"));

      SET_COMPILATION_STRATEGY =
          nativeLib
              .find("wasmtime4j_config_set_compilation_strategy")
              .map(
                  addr -> linker.downcallHandle(addr, FunctionDescriptor.ofVoid(ADDRESS, JAVA_INT)))
              .orElseThrow(
                  () -> new UnsatisfiedLinkError("wasmtime4j_config_set_compilation_strategy"));

      SET_OPTIMIZATION_LEVEL =
          nativeLib
              .find("wasmtime4j_config_set_optimization_level")
              .map(
                  addr -> linker.downcallHandle(addr, FunctionDescriptor.ofVoid(ADDRESS, JAVA_INT)))
              .orElseThrow(
                  () -> new UnsatisfiedLinkError("wasmtime4j_config_set_optimization_level"));

      SET_RESOURCE_LIMITS =
          nativeLib
              .find("wasmtime4j_config_set_resource_limits")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr,
                          FunctionDescriptor.ofVoid(
                              ADDRESS, JAVA_LONG, JAVA_INT, JAVA_INT, JAVA_INT, JAVA_INT)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_set_resource_limits"));

      ENABLE_FEATURE =
          nativeLib
              .find("wasmtime4j_config_enable_feature")
              .map(
                  addr ->
                      linker.downcallHandle(
      ...
      DISABLE_FEATURE =
          nativeLib
              .find("wasmtime4j_config_disable_feature")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr, FunctionDescriptor.of(JAVA_BOOLEAN, ADDRESS, ADDRESS)))
      ...
      SET_PARAMETER =
          nativeLib
              .find("wasmtime4j_config_set_parameter")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr, FunctionDescriptor.of(JAVA_BOOLEAN, ADDRESS, ADDRESS, ADDRESS)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_set_parameter"));

      SET_MEMORY_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_set_memory_config")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr,
                          FunctionDescriptor.ofVoid(
                              ADDRESS, JAVA_LONG, JAVA_LONG, JAVA_LONG, JAVA_BOOLEAN, JAVA_LONG)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_set_memory_config"));

      SET_THREAD_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_set_thread_config")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr,
                          FunctionDescriptor.ofVoid(
                              ADDRESS, JAVA_INT, JAVA_LONG, JAVA_BOOLEAN, JAVA_INT, JAVA_LONG)))
              .orElseThrow(() -> new UnsatisfiedLinkError("wasmtime4j_config_set_thread_config"));

      SET_DEBUG_CONFIGURATION =
          nativeLib
              .find("wasmtime4j_config_set_debug_config")
              .map(
                  addr ->
                      linker.downcallHandle(
                          addr,
  ...
  private Set<String> enabledFeatures =
      EnumSet.noneOf(WasmFeature.class).stream()
          .map(Enum::name)
  ...
  public PanamaAdvancedConfiguration() {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINE,
          "Created Panama advanced configuration with handle: {0}",
          nativeConfigHandle.address());
    } catch (final Throwable e) {
      arena.close();
      throw PanamaExceptionHandler.wrapException(
          e, "Failed to create Panama advanced configuration");
    }
  }
  ...
  public AdvancedConfiguration setResourceLimits(final ResourceLimits limits) {
    ...
    try {
      SET_RESOURCE_LIMITS.invokeExact(
          nativeConfigHandle,
  ...
  public AdvancedConfiguration setParameter(final String key, final Object value) {
    ...
    try {
      ...
      final boolean success =
          (boolean) SET_PARAMETER.invokeExact(nativeConfigHandle, keyStr, valueStr);
  ...
  public <T> T getParameter(final String key, final Class<T> type) {
    ...
    throw new IllegalArgumentException(
        "Cannot convert parameter " + key + " to type " + type.getName());
  }
  ...
  public PanamaAdvancedConfiguration setMemoryConfiguration(final MemoryConfiguration config) {
    ...
    try {
      SET_MEMORY_CONFIGURATION.invokeExact(
          nativeConfigHandle,
  ...
  public PanamaAdvancedConfiguration setThreadConfiguration(final ThreadConfiguration config) {
    ...
    try {
      SET_THREAD_CONFIGURATION.invokeExact(
          nativeConfigHandle,
  ...
  public PanamaAdvancedConfiguration setDebugConfiguration(final DebugConfiguration config) {
    ...
    try {
      SET_DEBUG_CONFIGURATION.invokeExact(
          nativeConfigHandle,
  ...
  private void validateNotApplied() {
    if (applied) {
      throw new IllegalStateException(
          "Configuration has already been applied and cannot be modified");
    }
  }
  ...
  public static final class MemoryConfiguration {
    ...
    public long getInitialMemorySize() {
      return initialMemorySize;
    }

    public MemoryConfiguration setInitialMemorySize(final long size) {
      this.initialMemorySize = size;
      return this;
    }
    ...
    public MemoryConfiguration setMaximumMemorySize(final long size) {
      this.maximumMemorySize = size;
      return this;
    }

    public long getMemoryGrowthIncrement() {
      return memoryGrowthIncrement;
    }

    public MemoryConfiguration setMemoryGrowthIncrement(final long increment) {
      this.memoryGrowthIncrement = increment;
      return this;
    }

    public boolean isMemoryGuardEnabled() {
      return memoryGuardEnabled;
    }

    public MemoryConfiguration setMemoryGuardEnabled(final boolean enabled) {
    ...
    public MemoryConfiguration setMemoryGuardSize(final long size) {
      this.memoryGuardSize = size;
      return this;
    }
    ...
    public String toString() {
      return String.format(
          "MemoryConfiguration{initial=%d, max=%d, increment=%d, guard=%s, guardSize=%d}",
          initialMemorySize,
          maximumMemorySize,
          memoryGrowthIncrement,
          memoryGuardEnabled,
          memoryGuardSize);
    }
  }
  ...
  public static final class ThreadConfiguration {
    ...
    public ThreadConfiguration setMaxThreads(final int threads) {
    ...
    public ThreadConfiguration setThreadStackSize(final long size) {
    ...
    public ThreadConfiguration setThreadPoolEnabled(final boolean enabled) {
      this.threadPoolEnabled = enabled;
      return this;
    }
    ...
    public ThreadConfiguration setThreadPoolSize(final int size) {
    ...
    public ThreadConfiguration setThreadAffinityMask(final long mask) {
      this.threadAffinityMask = mask;
      return this;
    }
    ...
    public String toString() {
      return String.format(
          "ThreadConfiguration{maxThreads=%d, stackSize=%d, poolEnabled=%s, poolSize=%d,"
              + " affinity=0x%x}",
  ...
  public static final class DebugConfiguration {
    ...
    public DebugConfiguration setDebugInfoEnabled(final boolean enabled) {
    ...
    public DebugConfiguration setProfilingEnabled(final boolean enabled) {
      this.profilingEnabled = enabled;
      return this;
    }

    public Duration getProfilingSamplingInterval() {
      return profilingSamplingInterval;
    }

    public DebugConfiguration setProfilingSamplingInterval(final Duration interval) {
      this.profilingSamplingInterval = interval;
      return this;
    }
    ...
    public DebugConfiguration setStackTraceEnabled(final boolean enabled) {
    ...
    public DebugConfiguration setMaxStackTraceDepth(final int depth) {
    ...
    public String toString() {
      return String.format(
          "DebugConfiguration{debugInfo=%s, profiling=%s, samplingInterval=%s, stackTrace=%s,"
              + " maxDepth=%d}",
          debugInfoEnabled,
          profilingEnabled,
          profilingSamplingInterval,
          stackTraceEnabled,
          maxStackTraceDepth);
    }
  }
