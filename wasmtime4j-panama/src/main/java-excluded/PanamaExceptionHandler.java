...
public final class PanamaExceptionHandler {
  ...
  public MemorySegment createNativeHandler(final ExceptionHandler.ExceptionHandlingConfig config) {
    ...
    try {
      ...
      LOGGER.fine("Created native exception handler at address: " + handlerSegment.address());
  ...
  public long createNativeExceptionTag(
    ...
    PanamaValidation.validateNotNull(handlerSegment, "Handler segment");
    ...
    try {
      ...
      final MemorySegment nameSegment = arena.allocateUtf8String(name.trim());

      // Convert parameter types to byte array
      final MemorySegment typesSegment =
          arena.allocateArray(ValueLayout.JAVA_BYTE, parameterTypes.size());
      ...
      final ExceptionHandler.ExceptionTag tag =
          new ExceptionHandler.ExceptionTag(
              name.trim(), parameterTypes, tagHandle, handlerSegment.address());
  ...
  public String captureStackTrace(final MemorySegment handlerSegment, final long tagHandle) {
    ...
    try {
      ...
      final String trace = traceSegment.getUtf8String(0);
      ...
      tagCache
          ...
          .removeIf(
              entry -> {
                ...
                return tag.getHandlerId() == handlerAddress;
              });
