 ...
 * 
 * <p>These tests validate the performance optimizations, zero-copy operations, memory-mapped
 * file support, caching mechanisms, and bulk processing capabilities implemented for Issue #10.
 */
@EnabledOnJre(value = {JRE.JAVA_23}, disabledReason = "Panama FFI requires Java 23+")
class PanamaModuleHighPerformanceTest {
  ...
  void testZeroCopyCompilation() throws Exception {
    ...
    ArenaResourceManager.ManagedMemorySegment wasmMemory = resourceManager.allocate(SIMPLE_WASM.length);
  ...
  void testOptimizedMetadataExtraction() throws Exception {
    ...
    assertTrue(secondCallTime <= firstCallTime || secondCallTime < 100_000, // 100μs threshold
        "Cached call should be faster or very fast (first=" + firstCallTime + "ns, second=" + secondCallTime + "ns)");
  ...
  void testModuleByteCaching() throws Exception {
    ...
    assertArrayEquals(SIMPLE_WASM, cachedData.serializedModule, "Cached module bytecode should match");
  ...
  void testBulkModuleOperations() throws Exception {
    ...
    for (int i = 0; i < compiledModules.length; i++) {
      ...
      assertNotNull(compiledModules[i].getModulePointer(), "Module " + i + " should have valid pointer");
    }
    ...
    assertTrue(averageBulkTime <= individualCompileTime * 1.5, // Allow 50% overhead tolerance
        "Bulk compilation should be efficient (bulk avg=" + averageBulkTime + "ns, individual=" + individualCompileTime + "ns)");
  ...
  void testHighPerformanceModuleSerialization() throws Exception {
    ...
    assertArrayEquals(standardSerialized, serializedData, "Both serialization methods should return same data");
  ...
  void testErrorHandling() throws Exception {
    ...
    assertThrows(NullPointerException.class, () -> 
        PanamaModule.compileFromMappedFile(null, tempWasmFile));
    
    assertThrows(NullPointerException.class, () -> 
        PanamaModule.compileFromMappedFile(engine, null));
  ...
  /**
   * Performance benchmark test to validate 20%+ improvement claims.
   * Note: This is a basic performance test - full benchmarks should use JMH.
   */
  @Test
  @Timeout(value = 30, unit = TimeUnit.SECONDS)
  void testPerformanceBenchmark() throws Exception {
    ...
    assertTrue(improvementRatio >= 1.2, // 20% improvement minimum
