...
/**
 * Comprehensive tests for StreamingExporter functionality including async operations,
...
class StreamingExporterTest {
  ...
  @BeforeEach
  void setUp() {
    final StreamingConfiguration config = new StreamingConfiguration.Builder()
        .chunkSize(50)
        .bufferSize(4096)
        .chunkDelayMs(10)
        .threadPoolSize(2)
  ...
  void shouldExportSynchronouslyWithStreamingSupport() throws IOException, ExportException {
    ...
    // When
    final ExportResult result = streamingExporter.exportSync(
        testReport,
        jsonReporter,
        config,
        outputStream,
  ...
  void shouldExportAsynchronouslyWithProgressReporting() throws Exception {
    ...
    final CsvConfiguration config = new CsvConfiguration.Builder()
    ...
    // When
    final CompletableFuture<ExportResult> future = streamingExporter.exportAsync(
        largeReport,
        csvReporter,
        config,
        outputStream,
  ...
  void shouldTrackBytesWrittenAccurately() throws IOException, ExportException {
    ...
    assertTrue(Math.abs(actualBytes - reportedBytes) <= 100,
  ...
  private ComparisonReport createTestReport() {
    ...
    final ReportSummary summary = new ReportSummary(
        3, 3, 3, 3, 0, 0, 0.95, Map.of("CONSISTENT", 3)
    );
  ...
  private ComparisonReport createLargeTestReport(final int testCount) {
    ...
    final ReportSummary summary = new ReportSummary(
        testCount, testCount, testCount, testCount, 0, 0, 0.9, Map.of()
    );
  ...
  private static class TestNonStreamingReporter implements DataExporter<JsonConfiguration> {
    @Override
    public void export(ComparisonReport report, JsonConfiguration configuration,
                      java.io.OutputStream output) throws IOException, ExportException {
