...
/**
 * Comprehensive test suite for InsightGenerator ensuring accurate pattern recognition, trend
...
class InsightGeneratorTest {
  ...
  private void setupTestData() {
    ...
    behavioralResults =
        new BehavioralAnalysisResult.Builder("insight_test")
            .pairwiseComparisons(Collections.emptyList())
            .discrepancies(
                List.of(
                    createDiscrepancy("Performance affecting issue", DiscrepancySeverity.MAJOR)))
    ...
    final CoverageMetrics coverageMetrics =
        new CoverageMetrics(
            10, 8, 80.0, Map.of(RuntimeType.JNI, 75.0, RuntimeType.PANAMA, 85.0), 90.0);

    coverageResults =
        new CoverageAnalysisResult.Builder("insight_test")
            ...
            .featureInteractionAnalysis(
                new FeatureInteractionAnalysis(
                    Map.of("memory+simd", Set.of("memory", "simd")), Collections.emptyList(), 4.5))
            .build();

    // Setup recommendation results
    final List<ActionableRecommendation> recommendations = new ArrayList<>();
    recommendations.add(
        createRecommendation("Fix JNI Performance", IssueCategory.PERFORMANCE, IssueSeverity.HIGH));
    recommendations.add(
        createRecommendation("Improve Coverage", IssueCategory.COVERAGE, IssueSeverity.MEDIUM));

    final RecommendationSummary summary =
        new RecommendationSummary(
            2, 1, 1, 0, Map.of(IssueCategory.PERFORMANCE, 1, IssueCategory.COVERAGE, 1));

    recommendationResults =
        new RecommendationResult.Builder("insight_test")
  ...
  private Map<String, PerformanceAnalyzer.PerformanceMetrics> createPerformanceMetrics() {
    ...
    jniResults.add(
        PerformanceAnalyzer.TestExecutionResult.builder("test", "JNI")
    ...
    panamaResults.add(
        PerformanceAnalyzer.TestExecutionResult.builder("test", "PANAMA")
  ...
  private BehavioralDiscrepancy createDiscrepancy(
      final String description, final DiscrepancySeverity severity) {
    return new BehavioralDiscrepancy(
        DiscrepancyType.PERFORMANCE_RELATED, description, severity, Set.of(RuntimeType.JNI));
  }
  ...
  private ActionableRecommendation createRecommendation(
      final String title, final IssueCategory category, final IssueSeverity severity) {
  ...
  class InsightGenerationTests {
    ...
    void shouldGenerateComprehensiveInsights() {
      final InsightAnalysisResult result =
    ...
    void shouldGeneratePerformanceOptimizationInsights() {
      ...
      final boolean hasVariabilityInsight =
          performanceInsights.stream()
              .anyMatch(
                  insight ->
                      insight.getType() == PerformanceInsightType.HIGH_VARIABILITY
                          && insight.getRuntime().equals("JNI"));
      assertTrue(hasVariabilityInsight);

      // Should detect memory spikes
      final boolean hasMemorySpike =
          performanceInsights.stream()
    ...
    void shouldGenerateCrossCuttingInsights() {
      ...
      final BehavioralAnalysisResult poorBehavioral =
          new BehavioralAnalysisResult.Builder("cross_cutting_test")
              .pairwiseComparisons(Collections.emptyList())
              .discrepancies(
                  List.of(createDiscrepancy("Major issue", DiscrepancySeverity.CRITICAL)))
      ...
      final boolean hasCorrelationInsight =
    ...
    void shouldHandleNullInputsGracefully() {
      ...
      assertThrows(
          ...
          () ->
              insightGenerator.generateInsights(
                  "test", null, performanceResults, coverageResults, recommendationResults));

      assertThrows(
          NullPointerException.class,
          () ->
              insightGenerator.generateInsights(
                  "test", behavioralResults, null, coverageResults, recommendationResults));

      assertThrows(
          NullPointerException.class,
          () ->
              insightGenerator.generateInsights(
                  "test", behavioralResults, performanceResults, null, recommendationResults));

      assertThrows(
          NullPointerException.class,
          () ->
              insightGenerator.generateInsights(
                  "test", behavioralResults, performanceResults, coverageResults, null));
    }
  }
  ...
  class BatchInsightGenerationTests {
    ...
    void shouldGenerateExecutiveInsights() {
      ...
      final boolean hasSystemHealthInsight =
          executiveInsights.stream()
    ...
    void shouldHandleEmptyTestResults() {
      final BatchInsightResult result =
          insightGenerator.generateBatchInsights(Collections.emptyMap());
  ...
  class OptimizationRecommendationsTests {
    ...
    void shouldProvideOptimizationRecommendations() {
      ...
      final List<OptimizationRecommendation> recommendations =
          insightGenerator.getOptimizationRecommendations();
    ...
    void shouldPrioritizeOptimizationRecommendations() {
      ...
      final List<OptimizationRecommendation> highConfidence =
          recommendations.stream().filter(rec -> rec.getConfidenceScore() > 0.75).toList();

      for (final OptimizationRecommendation rec : highConfidence) {
        assertTrue(
            rec.getPriority() == OptimizationPriority.HIGH
                || rec.getPriority() == OptimizationPriority.MEDIUM);
      }
    }
  }
  ...
  class ConfidenceAndQualityMetricsTests {
    ...
    void shouldCountInsightsCorrectly() {
      ...
      final int expectedCount =
          result.getPerformanceInsights().size()
  ...
  class EdgeCasesAndRobustnessTests {
    ...
    void shouldHandleMinimalDataGracefully() {
      ...
      final BehavioralAnalysisResult minimalBehavioral =
          new BehavioralAnalysisResult.Builder("minimal_test")
              .pairwiseComparisons(Collections.emptyList())
      ...
      final CoverageAnalysisResult minimalCoverage =
          new CoverageAnalysisResult.Builder("minimal_test")
      ...
      final RecommendationResult minimalRecommendations =
          new RecommendationResult.Builder("minimal_test")
              .behavioralRecommendations(Collections.emptyList())
    ...
    void shouldHandleExtremePerformanceVariability() {
      ...
      extremeResults.add(
          PerformanceAnalyzer.TestExecutionResult.builder("extreme_test", "JNI")
